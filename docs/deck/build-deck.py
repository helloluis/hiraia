...
# Brand (hiraia.org + card-ui `card` palette): paper ground, deep-green ink, chalkboard, card stock, gold, oxblood.
PAPER, INK, PRIMARY, ACCENT, COVER = "#fdfdf6", "#1C3B2E", "#20342C", "#D8A03A", "#F4EAD5"
OXBLOOD, TEAL, FORK_BLUE, OLIVE, SAGE, PEACH = "#7A2E22", "#2F6360", "#2B4C7E", "#5B6B52", "#8C9E6E", "#E7B08B"
MUTED = "#5B6B52"; CHART_TEAL, CHART_GOLD, CHART_GREY = "#2B4C7E", "#D8A03A", "#8C9E6E"; RAMP = ["#c9d3bd", "#8C9E6E", "#5B6B52", "#1C3B2E"]
# Typography: Alfa Slab One for slide titles (as on hiraia.org); sans-serif (Avenir Next) for every sentence and label.
F_TITLE, F_DISPLAY, F_BODY = "Alfa Slab One", "Inter", "Inter"
...
for ttf in ("AlfaSlabOne-Regular.ttf", "Inter-Regular.ttf", "Inter-SemiBold.ttf", "Inter-Medium.ttf"):
    fp = os.path.expanduser(f"~/Library/Fonts/{ttf}")
    if os.path.exists(fp): font_manager.fontManager.addfont(fp)
plt.rcParams.update({"font.family": ["Inter", "DejaVu Sans"], "font.size": 13, "axes.edgecolor": "#b9c6c9", "axes.labelcolor": INK, "xtick.color": INK, "ytick.color": INK, "axes.spines.top": False, "axes.spines.right": False, "figure.facecolor": PAPER, "axes.facecolor": PAPER, "savefig.facecolor": PAPER})
...
def new_slide(title, message, n):
    ...
    logo = os.path.join(D, "brand/icon.png")
    if os.path.exists(logo): s.shapes.add_picture(logo, Inches(0.45), Inches(0.3), height=Inches(0.6))
    text(s, 1.2, 0.3, 11.6, 0.7, title, F_TITLE, 26, PRIMARY)
    text(s, 0.5, 1.0, 12.3, 0.7, message, F_DISPLAY, 16, INK, bold=True)
    line = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.5), Inches(1.7), Inches(12.33), Emu(18000)); line.fill.solid(); line.fill.fore_color.rgb = rgb(ACCENT); line.line.fill.background()
    ...
    return s
def text(s, x, y, w, h, t, font, size, color, bold=False, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP, _display=None):
    bold = bold or (_display is True)
...
def stat(s, x, y, w, value, label):
    box = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(y), Inches(w), Inches(1.15)); box.fill.solid(); box.fill.fore_color.rgb = rgb(COVER); box.line.color.rgb = rgb(INK); box.line.width = Pt(1); box.adjustments[0] = 0.06
    text(s, x, y + 0.08, w, 0.65, value, F_TITLE, 24, INK, align=PP_ALIGN.CENTER); text(s, x, y + 0.7, w, 0.42, label, F_BODY, 11, INK, align=PP_ALIGN.CENTER)
def shot(s, x, y, h, name, caption):
    p = os.path.join(SHOTS, name); w = h * 0.466
    if os.path.exists(p): s.shapes.add_picture(p, Inches(x), Inches(y), height=Inches(h))
    else:
        ph = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(y), Inches(w), Inches(h)); ph.fill.solid(); ph.fill.fore_color.rgb = rgb(COVER); ph.line.color.rgb = rgb(INK); ph.line.width = Pt(1); ph.adjustments[0] = 0.06
    ...
    text(s, x - 0.3, y + h + 0.06, w + 0.6, 0.4, caption, F_BODY, 10.5, MUTED, align=PP_ALIGN.CENTER)
...
s = prs.slides.add_slide(BLANK); s.background.fill.solid(); s.background.fill.fore_color.rgb = rgb(PAPER)
logo = os.path.join(D, "brand/icon.png")
if os.path.exists(logo): s.shapes.add_picture(logo, Inches(0.9), Inches(0.75), height=Inches(1.2))
text(s, 0.9, 2.1, 11.5, 1.2, "Hiraia — progress update", F_TITLE, 46, PRIMARY); text(s, 0.9, 3.2, 11.5, 0.7, "Early September 2026", F_DISPLAY, 24, OXBLOOD, bold=True)
text(s, 0.9, 4.0, 9.2, 1.2, "An offline, on-device science tutor for Filipino grade-school students (Grades 3–10), aligned to the DepEd MATATAG curriculum, in Tagalog, Cebuano and English. The build targets Android 12+ phones with 6 GB or more memory, a single device tier: a 3B model with GPU offload, its Tagalog and Cebuano adapters bundled in the APK.", F_BODY, 15, INK)
for i, (v, l) in enumerate([(f"{cov['pool_cards']:,}", "illustrated fact cards"), (f"{cov['summary']['bank_facts']:,}", "verified trilingual facts"), (f"{cov['summary']['competencies']}", f"MATATAG competencies tracked ({cov.get('competencies_with_cards', 0)} with cards)"), (f"{cov['pool_cards_with_mcq']:,}", "cards with a quiz question")]): stat(s, 0.9 + i * 2.36, 5.35, 2.2, v, l)
...
# 4 feed
s = new_slide("A curriculum-aware feed", "Cards are weighted by the student's grade and the curriculum quarter inferred from the device date; the boost moves through the school year by itself, and flattens across the whole grade once school is out.", 4)
...
w = cov["weights"]; table(s, 0.5, 5.65, 7.4, [["current quarter, same grade", "adjacent quarter", "other quarter, same grade", "adjacent grade, current", "school out: any cell of the grade", "off-curriculum"], [f"×{w['current']}", f"×{w['adjacent']}", f"×{w['other_same_grade']}", f"×{w['adjacent_grade_current']}", f"×{w.get('school_out', 6)}", f"×{w['off']}"]], [1.35, 1.05, 1.35, 1.25, 1.55, 0.85], size=9)
...
if ec.get("ppl"):
    ...
    ax.barh([i - h/2 for i in y], [x[2] for x in ec["ppl"]], height=h, color=CHART_TEAL, linewidth=0)
    for i, x in enumerate(ec["ppl"]):
        lab_b = f" {x[1]:.1f}  base model" if i == 0 else f" {x[1]:.1f}"
        lab_a = f" {x[2]:.1f}  after continued pre-training" if i == 0 else f" {x[2]:.1f}"
        ax.text(x[1], i + h/2, lab_b, va="center", fontsize=10, color=INK)
        ax.text(x[2], i - h/2, lab_a, va="center", fontsize=10, color=CHART_TEAL)
    ax.set_yticks(list(y)); ax.set_yticklabels(langs); ax.invert_yaxis(); ax.set_xlabel("held-out perplexity (lower is better)"); ax.set_xlim(0, 34); ax.grid(axis="x", color="#e3eaeb", lw=0.8)
...
text(s, 0.5, 5.35, 5.8, 0.3, "Tiles: pedagogy figures are from the June 3B line; language accuracy and hedging from the August 2B (SFT v1). The 2B capability run is pending.", F_BODY, 9.5, MUTED)
tiles = ec.get("tiles", [])
for i, (v, l) in enumerate(tiles):
    x = 0.5 + i * 1.92; box = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(5.72), Inches(1.8), Inches(1.3)); box.fill.solid(); box.fill.fore_color.rgb = rgb(COVER); box.line.color.rgb = rgb(INK); box.line.width = Pt(1); box.adjustments[0] = 0.06
    text(s, x, 5.76, 1.8, 0.45, v, F_TITLE, 15, INK, align=PP_ALIGN.CENTER); text(s, x + 0.06, 6.18, 1.68, 0.8, l, F_BODY, 8.5, INK, align=PP_ALIGN.CENTER)
...
left = ["More SFT and knowledge distillation on the CPT base (mode-locked buckets, teacher-generated tutoring turns)", "Extensive synthetic testing on-device: scripted student sessions on target hardware, TTFT and per-page latency budgets", "Model-as-judge quality control on every generated fact, quiz and translation (decorrelated judge, AUP-safe routing)", "APK-level optimisations: bundle size (engravings subset, indexed PNG), cold start, SQLite migrations, edge-walk cost"]
...
text(s, 0.5, 6.1, 12.3, 0.4, "To report next time: the 2B capability run on the same 143 probes; on-device TTFT and per-page latency; quiz coverage after the lane completes; bundle size after optimisation.", F_DISPLAY, 13, PRIMARY, bold=True)
notes(s, "Plan items trace to: rag/pipeline/FACT-SWARM-SPEC.md (Lane B, quiz lane, module cards), rag/pipeline/FEED-WEIGHTING.md (Miss-Card labels, edge-walk cost), finetuning/eval/routing/README.md (hedge-clause wording sweep), finetuning/cpt/*.md (SFT/KD on the CPT base), on-device measurements on the Redmi SD685 (~4.4 tok/s, TTFT ~24 s) belong to the retired 1B budget tier; the project has since unified on a single 6 GB+ tier.")
# 7 links
...
for i, (k, v, note) in enumerate(L):
    text(s, 0.8, 2.0 + i * 1.15, 3.9, 0.6, k, F_TITLE, 17, PRIMARY); tb = text(s, 4.55, 2.03 + i * 1.15, 8.1, 0.6, v, F_BODY, 19, INK if v.startswith("http") else MUTED)
    if v.startswith("http"):
        _r = tb.text_frame.paragraphs[0].runs[0]; _r.hyperlink.address = v; _r.font.color.rgb = rgb(FORK_BLUE)
    if note: text(s, 4.55, 2.46 + i * 1.15, 8.1, 0.4, note, F_BODY, 11.5, MUTED)
