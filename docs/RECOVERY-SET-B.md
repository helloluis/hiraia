# Hiraia recovery set B

You own **2,836 facts** in partition `external_b`. Work in `/Users/luis/Code/hiraia-unified`, branch `unified`.
...
- Inventory: key `external_b` in `tools/curriculum-recovery/second-pass-partitions.json` (read-only).
- Your ledger: `tools/curriculum-recovery/external_b-second-pass-state.json`.
- Your packet range: `batch-20000` through `batch-29999`.
...
python3 tools/curriculum-recovery/second-pass-batch.py --owner external_b --limit 96 --out tools/curriculum-recovery/batch-20000.packet.json
```
