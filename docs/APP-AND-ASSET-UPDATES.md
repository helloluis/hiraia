...
Settings includes an explicit check. Notices can be dismissed for seven days per
release; a new release can notify again. No model or APK is downloaded just to check.
...
* Hiraia data: `assets` in the same schema-1 response, sourced from
  ...
  New APKs notify separately for compatible tutor-model weights and illustrations,
  showing the download size before consent. APK notices take priority.
...
Downloads pause when the app backgrounds and the visible Retry action resumes them.
...
selected. On Update, the catalog is saved and the existing illustration installer
handles downloads, progress and retries. A paused illustration preference stays paused;
resume from Settings. The update notice says queued, not installed. Files are extracted
