# Hiraia recovery set A

You own **2,836 facts** in partition `external_a`. Work in `/Users/luis/Code/hiraia-unified`, branch `unified`.
...
- Inventory: key `external_a` in `tools/curriculum-recovery/second-pass-partitions.json` (read-only).
- Your ledger: `tools/curriculum-recovery/external_a-second-pass-state.json`.
- Your packet range: `batch-10000` through `batch-19999`.
...
python3 tools/curriculum-recovery/second-pass-batch.py --owner external_a --limit 96 --out tools/curriculum-recovery/batch-10000.packet.json
```
