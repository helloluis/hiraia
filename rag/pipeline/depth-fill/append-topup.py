#!/usr/bin/env python3
"""Append depth-fill to the bank and carry it through the feed pipeline (copy of rag/pipeline/lane-a/append-lane-a.py, retargeted per depth-fill/BRIEF.md §8) (FACT-SWARM-SPEC.md, "stage + feed voice",
"exact tags", "pool + gate", "illustrate"). The consumer of ingest.py's emit stage.

Inputs (rag/pipeline/depth-fill/out/):
  depth-ingest-ready.jsonl           bank-schema rows + brief_code/card_form          (main stream)
  depth-ingest-ready.body.jsonl      same, the AUP stream — ids/counts only in any output, text never printed
  depth-tags.json                    curriculum-tags v2 fragment keyed by BANK fact id
...
Stages — every one is idempotent and resumable, and refuses to apply the same rows twice:
  bank      back up science-facts.jsonl + factoids.jsonl + curriculum-tags.json → rag/bank/.backup-<date>/ (first backup
            wins, self-ignored), strip brief_code/card_form → out/depth-bank-meta.json, append the rows (generator depth-fill,
            reviewed false); refuses if any depth-fill id is already banked (skips when ALL are: already applied)
  src       rag/pipeline/factoids-depth-fill-topup-src.jsonl in build-factoid-src.py's SRC schema, has_image=false for every row;
  ...
  set -a; . /Users/luis/Code/hiraia/.env.local; set +a          # FIREWORKS_API_KEY (gen), OPENAI_API_KEY (images)
  /opt/homebrew/bin/python3 rag/pipeline/depth-fill/append.py                       # all stages
  /opt/homebrew/bin/python3 rag/pipeline/depth-fill/append.py --stages bank,src     # a subset, in pipeline order
  /opt/homebrew/bin/python3 rag/pipeline/depth-fill/append.py --status              # what has been applied
...
FRAGMENT = os.path.join(OUT, 'depth-tags.json')
META_OUT = os.path.join(OUT, 'depth-bank-meta.json')
STATE = os.path.join(OUT, 'append-state.json')
BATCHES_OUT = os.path.join(OUT, 'image-batches.json')
SRC = os.path.join(ROOT, 'rag', 'pipeline', 'factoids-depth-fill-topup-src.jsonl')
TAG = 'depth-fill-topup'
GEN_DIR = os.path.join(ROOT, 'rag', 'pipeline', f'factoids-gen-{TAG}')
PAIR = f'rag/pipeline/factoids-gen-{TAG}:rag/pipeline/factoids-depth-fill-topup-src.jsonl'
...
VENV_PY = '/Users/luis/Code/hiraia/finetuning/.convert-venv/bin/python'     # the MAIN checkout's LaBSE venv (this worktree has none)
...
STREAMS = [('main', 'depth-ingest-ready.jsonl'), ('body', 'depth-ingest-ready.body.jsonl')]
AUP = {'body'}
...
def lane_rows():
    ...
    for stream, fn in STREAMS:
        ...
        if not rows:
            if stream == 'body':
                continue                      # top-up lane: no body-stream rows is the normal case
    ...
    if len(ids) != len(set(ids)):
        raise SystemExit('duplicate ids inside the depth-fill inputs')
    return out
...
def aup_strings():
    ...
    aup_ids = {i for s, ids in lane_ids_by_stream().items() if s in AUP for i in ids}
...
def stage_bank(st):
    ...
    if present and len(present) == len(ids):
        log(f'bank: all {len(ids)} depth-fill ids already banked (already applied) — nothing to do; bank {len(bank):,}')
        st['stages'].setdefault('bank', dict(done_at='(pre-existing)', appended=0, bank_after=len(bank)))
        save_state(st)
        return
    if present:
        raise SystemExit(f'bank: {len(present)}/{len(ids)} depth-fill ids are ALREADY in the bank (partial apply) — refusing; '
                         f'e.g. {present[:5]}')
    ...
    json.dump({'scheme': 'depth-fill side fields stripped from the bank rows at append (bank rows carry only the bank schema); '
                         'keyed by bank fact id', 'when': now(), 'rows': meta}, open(META_OUT, 'w'), indent=1)

    new_lines = []
    for _, r in rows:
        row = {k: r[k] for k in BANK_FIELDS}
        row['generator'] = 'depth-fill'
        row['reviewed'] = False
    ...
    for stream, ids in streams.items():
        ...
        for fn in glob.glob(os.path.join(GEN_DIR, 'gen-*.jsonl')):
            ...
            if len(keep) != len(rows):
                write_jsonl(fn, keep)
        # 2) move the fixed rows in (names are unique: gen-<tag>-<i>.jsonl never collides with gen-depth-fill-<i>.jsonl)
...
def stage_assemble(st, pairs_mode='default'):
    ...
    if not already:
        ...
        log(f'assemble: all {len(already)} depth-fill factIds already minted (already applied) — re-check only')
    else:
        log(f'assemble: {len(already)}/{len(lane)} depth-fill factIds already minted — the registry keeps those ids, '
    ...
    log(f'assemble: bank {len(head_ids):,} → {len(new):,}; new depth-fill factoids {len(new_rows)} '
...
def stage_tags(st):
    ...
    if len(fid_of) != len(lane):
        raise SystemExit(f'tags: only {len(fid_of)}/{len(lane)} depth-fill factoids minted — run assemble first')
    ...
    # prove the write is add-only: every pre-existing entry (loaded file AND backup) survives unchanged, depth-fill ids aside
...
def stage_images(st, chunk=120):
    ...
    if len(new_rows) != len(lane):
        raise SystemExit(f'images: {len(new_rows)}/{len(lane)} depth-fill factoids minted — run assemble first')
    ...
    for stream, rows in work.items():
        ...
        for k in range(n_chunks):
            ...
            created = mod._req(f'{api}/batches', data=json.dumps({
                ...
                'metadata': {'project': 'hiraia', 'lane': 'depth-fill', 'stream': stream, 'chunk': str(idx)}}).encode(),
            ...
            total_new += len(take)
            rec['scheme'] = ('depth-fill illustration batches (gpt-image-2 low, batch-submit-all.py request body, CHUNK≈120 so each '
...
def stage_fetch(st):
    ...
    rec['summary'].update(completed_batches=n_completed, declined=len(declined), webp_staged=sum(x.get('webp', 0) for x in rec['batches']),
                          fetched_at=now())
    rec['scheme'] = (rec.get('scheme', '').split(' Download:')[0] + ' The fetch stage of append.py books status/request_counts '
