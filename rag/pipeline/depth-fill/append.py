...
Stages — every one is idempotent and resumable, and refuses to apply the same rows twice:
  ...
  src       rag/pipeline/factoids-depth-fill-src.jsonl in build-factoid-src.py's SRC schema, has_image=false for every row;
...
SRC = os.path.join(ROOT, 'rag', 'pipeline', 'factoids-depth-fill-src.jsonl')
TAG = 'depth-fill'
GEN_DIR = os.path.join(ROOT, 'rag', 'pipeline', f'factoids-gen-{TAG}')
PAIR = f'rag/pipeline/factoids-gen-{TAG}:rag/pipeline/factoids-depth-fill-src.jsonl'
