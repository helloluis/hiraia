#!/usr/bin/env python3
"""Depth-fill ingest — rag/pipeline/lane-a/ingest-lane-a.py copied and retargeted per depth-fill/BRIEF.md §8.

Reads  out/depth-candidates.jsonl  (+ out/depth-body.jsonl, the AUP stream: counts only, text never printed)
and runs six resumable stages, every artefact under --out (default: rag/pipeline/depth-fill/out/):
  ...
  4 verify     Fireworks DeepSeek v4 Pro (decorrelated from the Flash writer; never gpt-oss), keep 'ok' only  → verify-verdicts/<stream>/ (cache), 4-verify.*, verify-drops.jsonl
  5 translate  en → tl + bis AND tl/bis/en query terms (DeepSeek v4 Pro); terms must hit both sentences  → translations.<stream>.json (cache),
               5-translate.*, translate-pending.jsonl
  6 emit       bank-schema rows → depth-ingest-ready.jsonl (+ .body twin) and the curriculum-tags v2 fragment depth-tags.json
...
(verify-verdicts/body/, translations.body.json) and its rows never share a Fireworks batch or a cache file with main rows;
...
and trilingual terms; --dry-run rows are stamped UNVERIFIED and a later non-dry-run translate/emit refuses them.
  ...
  set -a; . /Users/luis/Code/hiraia/.env.local; set +a          # FIREWORKS_API_KEY (not needed with --dry-run)
  PY=/Users/luis/Code/hiraia/finetuning/.convert-venv/bin/python   # torch + transformers (LaBSE, mps) — the MAIN checkout's venv
  $PY rag/pipeline/depth-fill/ingest.py --writer deepseek-v4-flash --dry-run     # stages 1-3 + 6, no Fireworks
  $PY rag/pipeline/depth-fill/ingest.py --writer deepseek-v4-flash --stages verify,translate,emit --conc 8 --batch 12
  FW_LIMIT=40 $PY rag/pipeline/depth-fill/ingest.py --writer deepseek-v4-flash --stages verify,translate,emit
  LANE_A_VERIFY_MODEL / FW_TRANSLATE_MODEL default to DeepSeek v4 Pro here; gpt-oss is refused outright (BRIEF §3)

Resume: stages 1–3 are deterministic and cheap (existing-set embeddings cached by file sha1 under depth-fill/.cache/), so they
...
DEEPSEEK_PRO = 'accounts/fireworks/models/deepseek-v4-pro-0813'          # judge + translator (BRIEF §3)
FORBIDDEN_MODEL = 'gpt-oss'                                              # never, in any role, this run
STAGES = ['validate', 'dedup', 'mint', 'verify', 'translate', 'emit']
STREAMS = [('main', 'depth-candidates.jsonl'), ('body', 'depth-body.jsonl')]
AUP_STREAMS = {'body'}          # text of these rows is never printed; only ids/counts
...
def validate_row(raw, briefs, stream, lineno, seen_tmp, seen_en, bank_en, en_words=EN_WORDS, min_terms=MIN_TERMS):
    reasons = []
    tmp_id = norm_text(raw.get('tmp_id') or '') or f'depth-{stream}-L{lineno}'
        ...
        with open(path, encoding='utf-8') as f:
            for lineno, line in enumerate(f, 1):
                ...
                try:
                    ...
                    rejects.append(dict(stage='validate', stream=stream, tmp_id=f'depth-{stream}-L{lineno}', line=lineno,
...
class Fireworks:
    ...
    def call(self, model, prompt, max_tokens, temperature, reasoning_effort=None, stage='fw', attempt=0, thinking=True):
        body = {'model': model, 'temperature': temperature, 'max_tokens': max_tokens,
                'messages': [{'role': 'user', 'content': prompt}]}
        if reasoning_effort and 'gpt-oss' in model:
            body['reasoning_effort'] = reasoning_effort
        if 'deepseek' in model:                       # fw-polish-language.py's switch: thinking on for the judge, off for the translator
            body['chat_template_kwargs'] = {'thinking': bool(thinking)}
        ...
        try:
            ...
            if e.code in (429, 500, 502, 503, 529) and attempt < 5:
                ...
                return self.call(model, prompt, max_tokens, temperature, reasoning_effort, stage, attempt + 1, thinking)
            raise
...
# verify prompt — verbatim from rag/pipeline/fw-verify-facts.py (only the judge model changes: DeepSeek v4 Pro, not the Flash writer)
    ...
    for stream, _ in STREAMS:
        ...
        if args.dry_run:
            ...
            def do_batch(facts):
                ...
                try:
                    c, rc = fw.call(args.verify_model, VERIFY_HEAD + json.dumps(payload, ensure_ascii=False), 12000, 0.1,
                                    reasoning_effort=args.verify_reasoning, stage='verify', thinking=True)
            ...
            def do_batch(facts):
                ...
                try:
                    c, rc = fw.call(args.translate_model, TRANSLATE_HEAD + json.dumps(payload, ensure_ascii=False), 6000, 0.2,
                                    reasoning_effort='low', stage='translate', thinking=False)
                except Exception as e:
                    with lock:
...
def stage_emit(args, briefs, rows_by_stream):
    ...
    if args.dry_run:
        source = f'depth-fill {args.writer} (dry-run, UNVERIFIED, {stamp})'
        ...
        source = f"depth-fill {args.writer} + {'/'.join(used)} verify ({stamp})"
    ...
    for stream, _ in STREAMS:
        ...
        for r in rows_by_stream.get(stream, []):
            ...
            row = dict(id=r['id'], domain=r['domain'], topic=r['topic'], grades=r['grades'], terms=r.get('terms_final') or r['terms'],
                       fact=dict(tl=r.get('tl', ''), en=r['en'], bis=r.get('bis', '')), source=source, generator='depth-fill',
            ...
            n_ready[stream] += 1
        fn = 'depth-ingest-ready.jsonl' if stream == 'main' else f'depth-ingest-ready.{stream}.jsonl'
        write_jsonl(os.path.join(args.out, fn), ready)
        log(f'[emit] {stream}: {len(ready)} rows → {fn}')
    write_json(os.path.join(args.out, 'depth-tags.json'), dict(
        scheme='depth-fill v2 fragment: born-labelled facts (codes=[brief_code], confidence 1.0), keyed by BANK fact id for the `bank` '
        ...
        bank=tags))
    log(f'[emit] {len(tags)} tag entries → depth-tags.json (curriculum-tags v2 fragment, bank section, keyed by fact id)')
    return n_ready
...
def main():
    ...
    ap.add_argument('--writer', default=os.environ.get('LANE_A_WRITER', 'deepseek-v4-flash'), help="the writing model, for source='depth-fill <writer> + <verify model> verify'")
    ...
    ap.add_argument('--verify-model', default=os.environ.get('LANE_A_VERIFY_MODEL', DEEPSEEK_PRO),
    ...
    ap.add_argument('--translate-model', default=os.environ.get('FW_TRANSLATE_MODEL', DEEPSEEK_PRO))
    ...
    if not args.dry_run and any(FORBIDDEN_MODEL in m for m in (args.verify_model, args.translate_model)):
        sys.exit(f'refusing: {FORBIDDEN_MODEL} may not verify or translate in depth-fill (BRIEF §3) — use {DEEPSEEK_PRO}')
    ...
    log(f'depth-fill ingest | out={args.out} | briefs {len(briefs)} | bank {len(bank)} | {"DRY RUN" if args.dry_run else "Fireworks: verify=" + args.verify_model + " translate=" + args.translate_model}')
    fw = None if args.dry_run or not ({'verify', 'translate'} & set(stages)) else Fireworks(
        float(os.environ.get('FW_PRICE_IN', '1.20')), float(os.environ.get('FW_PRICE_OUT', '1.20')))     # ledger USD is an estimate; tokens are authoritative
    ...
    summary = dict(when=datetime.datetime.now().isoformat(timespec='seconds'), out=args.out, dry_run=args.dry_run, writer=args.writer,
                   ...
                   ready={stream: len(read_jsonl(os.path.join(args.out, 'depth-ingest-ready.jsonl' if stream == 'main' else f'depth-ingest-ready.{stream}.jsonl'))) for stream, _ in STREAMS})
    if fw:
        i, o, cost = fw.cost()
    ...
    if 'emit' in stages and args.samples:
        ready = read_jsonl(os.path.join(args.out, 'depth-ingest-ready.jsonl'))
