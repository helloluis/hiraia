...
id must never be re-minted or re-numbered:
  ...
  * post-assembly fixes and retirements live in the tracked overlay --patches
    ...
    "q": {lang: ..}, "domain": "LIVING_THINGS", "topic": "...", "retired": true}}}), applied
    after id assignment on every run. Retire a
...
def apply_patches(rows, patches):
    ...
    never revert it: `text` / `q` per-language replacements, `domain` / `topic` repairs, and
    `retired: true` tombstones (the row and id stay; gen-cards-pool.py skips it). Unknown ids or
    fields abort.

    `domain` and `topic` are here because they are joined from the SOURCE fact, and a factoid
    whose factId does not resolve in science-facts.jsonl silently gets both blank. A blank domain
    is not cosmetic downstream: it makes a one-card BY_DOMAIN bucket in cards.ts, so the card has
    no lateral edge and the feed can never leave it (wire-app-pool.py now refuses to wire one).
    Patching `domain` re-derives `subject` from it, so the row cannot end up self-inconsistent."""
    ...
    for i, p in patches.items():
        ...
        for k, v in p.items():
            if k in ('text', 'q'):
                ...
                if v not in SUBJECT:
                    raise SystemExit(f'patch {i}: domain {v!r} is not a MATATAG domain ({", ".join(sorted(SUBJECT))})')
                r['domain'] = v
                r['subject'] = SUBJECT[v]
            elif k == 'topic':
                if not (isinstance(v, str) and v.strip()):
                    raise SystemExit(f'patch {i}: topic must be a non-empty string')
                r['topic'] = v.strip()
            elif k == 'retired':
                if v:
                    r['retired'] = True
                ...
                raise SystemExit(f'patch {i}: unknown field {k!r} '
                                 '(allowed: text, q, domain, topic, retired)')
    return len(patches)
