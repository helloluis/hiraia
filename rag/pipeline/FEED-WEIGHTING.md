...
- **DepEd module provenance** (`gen-curriculum-tags.mjs`, second source) — a `dcard-*` card is written
  from a DepEd learning module, and that module names its grade and usually its quarter. Where the
  labeler produced nothing, the card is tagged from that provenance instead of being dropped into
  off-curriculum: one strong cell at `confidence 0.5` (grade known but quarter unknown → all four
  quarters of that grade, WEAK, so each is capped at the adjacent-quarter level). The seen-group is
  the MODULE (median 4 cards) so decay stays at competency scale; the `norm` denominator is the CELL,
  so the provenance route cannot swamp the band it claims. Without this, 54% of the DepEd half of the
  deck sat in the ×0.4 band and the module-derived cards — the ones tied to what a class is actually
  being taught — were the least likely to be shown (21.3% of the pool, 13.5% of draws; now 20.0%).
