...
def arg(k, d=None):
    i = sys.argv.index(f'--{k}') if f'--{k}' in sys.argv else -1
    return sys.argv[i + 1] if i > -1 else d


SRC = os.path.join(HERE, 'raw')
DST = arg('out', os.path.join(HERE, 'webp'))
...
def main():
    ...
    wanted = {l.strip() for l in open(ONLY) if l.strip()} if ONLY else None
    done = skipped = 0
    for p in src:
