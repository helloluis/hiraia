#!/usr/bin/env python3
"""Audit-3 v4: reversible option-check labels and quarantined prior attempts.
...
import gemini_audit3_batch_v3 as PREV
...
SCHEMA_ENCODING = 'explicit_enum_string_type_strict_option_underscore_v4'
PROMPT = A.e.BASE + '\n' + A.e.DIAG
...
def dependency_hashes():
    ...
    todo, seen, paths = [A, B, C, PREV, K], set(), {Path(__file__).resolve()}
    while todo:
        module = todo.pop()
...
def explicit_enum_strings(schema):
    """Return the same schema with only implied string types made explicit.

    A nonempty enum consisting solely of strings already rejects every nonstring
    JSON value. Adding type:string therefore changes encoding, not acceptance.
    Existing type constraints (including unusual combinations) are never changed.
    This copy is used only in the wire schema, never in the prompt's schema.
    """
    if isinstance(schema, dict):
        value = {key: explicit_enum_strings(child) for key, child in schema.items()}
        choices = value.get('enum')
        if ('type' not in value and isinstance(choices, list) and choices
                and all(isinstance(choice, str) for choice in choices)):
            value['type'] = 'string'
        return value
    if isinstance(schema, list):
        return [explicit_enum_strings(child) for child in schema]
    return schema


def payload(job):
    # Labels are a reversible encoding only. Both schema occurrences use the
    # same labels; the quiz, system message and all settings stay unchanged.
    return K.adapt_payload(PREV.payload(job), len(job['content']['target']['options']))
...
def predecessor_snapshot(predecessor_out, jobs_path, integration_manifest, baseline):
    """Freeze a stopped, entirely rejected v3 epoch and its complete prior history.

    This bounded migration deliberately rejects ambiguous/live/successful prior
    epochs. It carries the full unknown reserve for every all-failed request and
    prevents resubmission of every durable prior attempt.
    """
    prior = Path(predecessor_out).resolve()
    if not (prior / 'BLOCKED.json').is_file():
        raise ValueError('Predecessor must be a blocked v3 run')
    process = read(prior / 'process.json')
    if process.get('status') not in ('stopped', 'finished'):
        raise ValueError('Predecessor process must be stopped or finished')
    config, jobs, _ = PREV.frozen(prior)
    if (Path(config['jobs']) != jobs_path or Path(config['integration_manifest']) != integration_manifest
            or config['baseline_accounted_usd'] != baseline):
        raise ValueError('Predecessor integration scope or original baseline differs')
    seen = PREV.check_ledger(prior, jobs)
    if not seen:
        raise ValueError('Predecessor has no durable attempts to quarantine')
    ancestor = PREV.verify_predecessor(prior)
    files = {prior / name for name in ('config.json', 'plan.json', 'BLOCKED.json', 'process.json', 'predecessor.json')}
    files.update(Path(path) for path in ancestor['files_sha256'])
    # Frozen run config binds its original code, prompt, integrated jobs and refs.
    files.update(Path(path) for path in config['dependencies_sha256'])
    files.update((jobs_path, integration_manifest))
    batches = PREV.batch_dirs(prior)
    if not batches:
        raise ValueError('Predecessor has no v3 attempted batch')
    for directory in batches:
        _, envelope = PREV.checked_batch(prior, directory, jobs)
        if not (directory / 'terminal.json').is_file():
            raise ValueError('Predecessor has live or ambiguous remote work')
        accepted = read(directory / 'accepted.json')
        PREV.validate_remote(accepted, envelope)
        terminal = read(directory / 'terminal.json')
        PREV.validate_remote(terminal, envelope, accepted['id'])
        n = len(envelope['requests'])
        if (terminal['status'] != 'failed'
                or terminal.get('request_counts') != {'total': n, 'completed': 0, 'failed': n}
                or terminal.get('usage') is not None or terminal.get('results') is not None
                or not terminal.get('error')
                or (directory / 'billing.json').exists() or (directory / 'collected.json').exists()):
            raise ValueError('Predecessor is not an entirely rejected, unreconciled epoch')
        files.update(directory / name for name in ('manifest.json', 'request.json',
            'submission-intent.json', 'accepted.json', 'terminal.json'))
        files.update(directory / name for name in ('submission-response.json', 'submission-error.json')
                     if (directory / name).exists())
    results = sorted(path for path in (prior / 'results').rglob('*') if path.is_file())
    if results:
        raise ValueError('Unexpected predecessor result or attempt artifacts')
    prior_cost = PREV.accounting(prior)
    if prior_cost['received_cost_usd'] != 0 or prior_cost['active_reserved_usd'] != 0:
        raise ValueError('Predecessor charges or active work require separate reconciliation')
    keys = [key for key in jobs if key in seen]
    by_language = collections.Counter(jobs[key]['content']['language'] for key in keys)
    historical_outputs = sorted({str(prior), ancestor['out'], ancestor['ancestor_out']})
    return {'version': 3, 'out': str(prior), 'ancestor_out': ancestor['out'],
        'historical_outputs': historical_outputs,
        'runner': str(Path(PREV.__file__).resolve()),
        'runner_sha256': sha(PREV.__file__),
        'files_sha256': {str(path): sha(path) for path in sorted(files)},
        'batch_paths': [str(path) for path in batches], 'result_paths': [],
        'attempted_keys': keys, 'technical_errors': len(keys), 'unresolved_requests': len(keys),
        'by_language': {lang: by_language[lang] for lang in ('tl', 'bis')},
        'received_cost_usd': prior_cost['received_cost_usd'],
        'received_usage': prior_cost['received_usage'],
        'uncertain_charge_reserved_usd': prior_cost['uncertain_charge_reserved_usd'],
        'active_reserved_usd': prior_cost['active_reserved_usd'],
        'budget_accounted_usd': prior_cost['budget_accounted_usd'],
        'billing_note': 'All prior epochs returned null usage/results; their cumulative reservation remains unknown, not zero charged.'}


@functools.lru_cache(maxsize=512)
def _frozen_sha(path, size, mtime_ns, ctime_ns, inode):
    return sha(path)


def verify_predecessor(out):
    config = read(out / 'config.json')
    path = out / 'predecessor.json'
    if sha(path) != config['predecessor_sha256']:
        raise ValueError('Frozen predecessor record changed')
    prior = read(path)
    if (prior['out'] != config['predecessor_out'] or prior['attempted_keys'] != config['excluded_keys']
            or prior['budget_accounted_usd'] != config['predecessor_accounted_usd']):
        raise ValueError('Frozen predecessor configuration differs')
    for name, expected in prior['files_sha256'].items():
        path = Path(name); st = path.stat()
        if _frozen_sha(str(path), st.st_size, st.st_mtime_ns, st.st_ctime_ns, st.st_ino) != expected:
            raise ValueError('Frozen predecessor artifact changed: ' + str(path))
    old_out = Path(prior['out'])
    # Check ancestor directory inventories too: flattening file hashes alone
    # would miss a newly attempted batch in an older output directory.
    PREV.verify_predecessor(old_out)
    if [str(path) for path in PREV.batch_dirs(old_out)] != prior['batch_paths']:
        raise ValueError('Predecessor acquired new batches after migration')
    if any(path.is_file() for path in (old_out / 'results').rglob('*')):
        raise ValueError('Predecessor acquired result or attempt artifacts after migration')
    for directory in PREV.batch_dirs(old_out):
        if (directory / 'billing.json').exists() or (directory / 'collected.json').exists():
            raise ValueError('Predecessor billing changed after migration')
    return prior


def make_plan(jobs, excluded):
    excluded = set(excluded)
    return [{'key': key, 'job_sha256': digest(job), 'target_sha256': digest(job['content']['target']),
             'request_sha256': digest(payload(job)), 'schema_sha256': digest(payload(job)['response_format'])}
            for key, job in jobs.items() if key not in excluded]


def prepare(out, jobs_path, integration_manifest, baseline_accounted_usd, predecessor_out):
    if not number(baseline_accounted_usd) or baseline_accounted_usd + BUDGET > COMBINED_BUDGET:
        raise ValueError('Combined baseline plus Audit-3 ceiling exceeds authorization')
    if (out / 'config.json').exists() or (out / 'batches').exists() or (out / 'results').exists():
        raise ValueError('Run already exists; prepare never changes a prior request epoch')
    jobs_path = jobs_path.resolve(); integration_manifest = integration_manifest.resolve()
    predecessor_out = predecessor_out.resolve()
    if out.resolve() == predecessor_out or out.resolve().is_relative_to(predecessor_out) or predecessor_out.is_relative_to(out.resolve()):
        raise ValueError('New output must be separate from predecessor output')
    jobs = load_jobs(jobs_path)
    validate_integration(jobs_path, integration_manifest, jobs)
    prior = predecessor_snapshot(predecessor_out, jobs_path, integration_manifest, baseline_accounted_usd)
    for previous_out in map(Path, prior['historical_outputs']):
        if out.resolve().is_relative_to(previous_out) or previous_out.is_relative_to(out.resolve()):
            raise ValueError('New output must be separate from all ancestor outputs')
    if prior['budget_accounted_usd'] >= BUDGET:
        raise ValueError('Predecessor exposure exhausted the cumulative Audit-3 ceiling')
    excluded = prior['attempted_keys']
    plan = make_plan(jobs, excluded)
    if not any(count >= CANARY_SIZE for count in collections.Counter(p['schema_sha256'] for p in plan).values()):
        raise ValueError('Need two untouched canary jobs with the same response schema')
    ensure(out / 'plan.json', plan)
    ensure(out / 'predecessor.json', prior)
    config = {'version': 4, 'schema_encoding': SCHEMA_ENCODING, 'diagnosis_encoding': K.ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
        'jobs': str(jobs_path), 'jobs_sha256': sha(jobs_path), 'selected': len(jobs),
        'eligible_selected': len(plan), 'excluded_keys': excluded,
        'predecessor_out': str(predecessor_out), 'predecessor_sha256': sha(out / 'predecessor.json'),
        'predecessor_accounted_usd': prior['budget_accounted_usd'],
...
def frozen(out):
    config = read(out / 'config.json')
    expected = {'version': 4, 'schema_encoding': SCHEMA_ENCODING, 'diagnosis_encoding': K.ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
        'model': A.MODEL, 'canonical_model': C.CANONICAL_MODEL, 'api': B.API, 'endpoint': '/v1/chat/completions',
    ...
    if any(config.get(k) != v or type(config.get(k)) is not type(v) for k, v in expected.items()):
        raise ValueError('Frozen configuration does not match authorized Audit-3 v4 settings')
    if not number(config.get('baseline_accounted_usd')) or config['baseline_accounted_usd'] + BUDGET > COMBINED_BUDGET:
        raise ValueError('Frozen cumulative budget baseline is invalid')
    if config['dependencies_sha256'] != dependency_hashes():
        raise ValueError('Frozen code dependency changed')
    if config['prompt_sha256'] != digest(PROMPT) or config['plan_sha256'] != sha(out / 'plan.json'):
        raise ValueError('Frozen prompt or plan changed')
    for name in ('jobs', 'integration_manifest'):
        if sha(config[name]) != config[name + '_sha256']:
            raise ValueError('Frozen ' + name + ' changed')
    prior = verify_predecessor(out)
    jobs = load_jobs(Path(config['jobs']))
    validate_integration(Path(config['jobs']), Path(config['integration_manifest']), jobs)
    plan = read(out / 'plan.json')
    projected = make_plan(jobs, prior['attempted_keys'])
    if (plan != projected or len(jobs) != config['selected'] or len(plan) != config['eligible_selected']
            or not set(prior['attempted_keys']).issubset(jobs)
            or len(jobs) != len(plan) + len(prior['attempted_keys'])):
        raise ValueError('Frozen plan or predecessor exclusion differs from integrated jobs')
...
def uncollected_batches(out):
    return [d for d in batch_dirs(out) if (d / 'submission-intent.json').exists()
            and not (d / 'collected.json').exists()]


def batch_state(directory):
    for name in ('terminal.json', 'status.json', 'accepted.json'):
        if (directory / name).exists():
            return read(directory / name).get('status')
    return 'ambiguous_submission' if (directory / 'submission-intent.json').exists() else 'unsubmitted'


def active_batches(out):
    # Accepted remote work stays active irrespective of local PID or elapsed
    # time. A confirmed terminal failure is never described as cloud pending.
    return [d for d in uncollected_batches(out)
            if (d / 'accepted.json').exists() and batch_state(d) not in TERMINAL]


def terminal_failures(out):
    return [d for d in uncollected_batches(out)
            if batch_state(d) in {'failed', 'cancelled', 'expired'}]
...
def accounting(out):
    ...
    for d in batch_dirs(out):
        ...
        elif (d / 'accepted.json').exists() and batch_state(d) not in TERMINAL:
            active += reserve
        else:
            unknown += reserve
    current = {'received_cost_usd': received, 'uncertain_charge_reserved_usd': unknown,
               'active_reserved_usd': active, 'budget_accounted_usd': received + unknown + active}
    received += prior['received_cost_usd']
    unknown += prior['uncertain_charge_reserved_usd']
    active += prior['active_reserved_usd']
    tokens.update(prior['received_usage'])
    total = received + unknown + active
    return {'predecessor_accounting': {name: prior[name] for name in (
                'received_cost_usd', 'uncertain_charge_reserved_usd', 'active_reserved_usd', 'budget_accounted_usd')},
            'current_accounting': current, 'received_cost_usd': received, 'received_usage': dict(tokens), 'uncertain_charge_reserved_usd': unknown,
...
def check_ledger(out, jobs):
    ...
    for directory in sorted((out / 'results').glob('*')):
        if not directory.is_dir():
            raise ValueError('Malformed current result or attempt artifacts')
        if any(directory.iterdir()):
            if directory.name in prior['attempted_keys']:
                raise ValueError('Quarantined predecessor key has new result artifacts')
            if directory.name not in seen:
                raise ValueError('Orphan current result or attempt artifacts; no resubmission')
    return seen


def canary_ready(out, jobs=None):
    ...
    for key in proof['keys']:
        final = verify_result(out, key, jobs, cache)
        if final is None:
            raise ValueError('Canary has no validated diagnosis')
        counts[final['status']] += 1
    if dict(counts) != collected['counts']:
        raise ValueError('Canary collected counts differ from validated native results')
    return True
...
def guard_admission(out):
    if terminal_failures(out):
        block(out, 'Terminal cloud failure requires reconciliation; no further submissions')
        raise ValueError('Terminal cloud failure requires reconciliation; no further submissions')
...
def next_keys(out, jobs, plan):
    ...
    if not canary and not canary_ready(out, jobs):
        return []
...
def submit(out, jobs, keys, network=B.network):
    ...
    if len(keys) > BATCH_SIZE:
        raise ValueError('Batch exceeds the frozen 500-request maximum')
    ...
    if batch_dirs(out) and not canary_ready(out, jobs):
...
def _batch_context(out, directory, jobs, cache=None):
    signatures = tuple(C._signature(directory / name) for name in ('request.json', 'accepted.json', 'terminal.json', 'manifest.json', 'submission-intent.json'))
    cache_key = (str(directory), signatures, sha(out / 'config.json'))
    ...
    value = {'manifest': manifest, 'envelope': envelope, 'accepted': accepted,
             'checked': checked, 'terminal_sha256': terminal_sha, 'bill': bill}
    if cache is not None:
        cache[cache_key] = value
    return value


def item_artifacts(out, directory, job, row, context):
    """Derive artifacts from native data; decode a copy only for A.validate.

    Neither this function nor the adapter ever modifies native response objects
    or message.content. Only the separate decoded-diagnosis artifact and final
    diagnosis use the original options[i] labels.
    """
    key = row['custom_id']; checked = context['checked']
    record = checked['results'][key]
    response = record.get('response') or {}
    raw, code = response.get('body'), response.get('status_code')
    final = None; error_record = None; fatal = False
    try:
        if record.get('error') or code != 200 or not isinstance(raw, dict):
            raise ValueError('Provider batch request failed')
        final = K.validate_wire_response(raw, clean_content(job))
    except (ValueError, KeyError, TypeError, IndexError) as error:
        provider_error = record.get('error') or {}
        fatal = code in (401, 402, 403, 404, 429) or (isinstance(provider_error, dict)
            and str(provider_error.get('code')) in ('401', '402', '403', '404', '429'))
        error_record = {'kind': 'transport' if code != 200 or record.get('error') else 'invalid_diagnosis',
            'error': str(error), 'http_status': code, 'provider_detail': record.get('error'),
            'fatal': fatal, 'automatic_retry': False, 'batch_id': context['accepted']['id']}
    raw_hash = digest(raw) if isinstance(raw, dict) else None
    try:
        content = raw['choices'][0]['message']['content']
    except (KeyError, TypeError, IndexError):
        content = None
    content_hash = hashlib.sha256(content.encode()).hexdigest() if isinstance(content, str) else None
    diagnosis_hash = digest(final['diagnosis']) if final is not None else None
    origin = {'kind': 'audit3_cloud_batch', 'batch_id': context['accepted']['id'], 'batch': directory.name,
        'key': key, 'config_sha256': sha(out / 'config.json'), 'job_sha256': digest(job),
        'target_sha256': digest(job['content']['target']),
        'request_file_sha256': sha(directory / 'request.json'), 'request_body_sha256': digest(row['body']),
        'accepted_file_sha256': sha(directory / 'accepted.json'), 'terminal_sha256': context['terminal_sha256'],
        'schema_encoding': SCHEMA_ENCODING, 'diagnosis_encoding': K.ENCODING,
        'adapter_sha256': sha(K.__file__), 'original_schema_sha256': digest(A.e.schema(clean_content(job), 'diagnose')),
        'wire_schema_sha256': digest(row['body']['response_format']['json_schema']['schema']),
        'response_body_sha256': raw_hash, 'response_content_sha256': content_hash,
        'canonical_diagnosis_sha256': diagnosis_hash}
    allocation = checked['allocations'][key]
    receipt = {'kind': 'allocation_of_actual_batch_charge', 'batch_id': context['accepted']['id'],
        'key': key, 'terminal_sha256': context['terminal_sha256'], 'origin_sha256': digest(origin),
        'response_body_sha256': raw_hash, 'batch_received_cost_usd': checked['received_cost_usd'],
        ...
        'usage_basis': allocation['usage_basis'], 'schema_encoding': SCHEMA_ENCODING,
        'diagnosis_encoding': K.ENCODING, 'canonical_diagnosis_sha256': diagnosis_hash}
    files = {'batch-origin.json': origin, 'batch-receipt.json': receipt,
             'request.json': row['body'], 'batch-result.json': record}
    if isinstance(raw, dict):
        files['response.json'] = raw
    if final is not None:
        files['decoded-diagnosis.json'] = {'diagnosis_encoding': K.ENCODING,
            'origin_sha256': digest(origin), 'response_body_sha256': raw_hash,
            'response_content_sha256': content_hash, 'canonical_diagnosis_sha256': diagnosis_hash,
            'diagnosis': final['diagnosis']}
        files['final.json'] = final
    else:
        files['error.json'] = error_record
    return files, final, fatal


def verify_result(out, key, jobs, cache=None):
    """Read and recompute native billing, decoding and every stored artifact."""
    local = out / 'results' / key
    origin = read(local / 'batch-origin.json')
    directory = out / 'batches' / origin['batch']
    context = _batch_context(out, directory, jobs, cache)
    if read(directory / 'billing.json') != context['bill']:
        raise ValueError('Terminal billing provenance changed')
    rows = [row for row in context['envelope']['requests'] if row['custom_id'] == key]
    if len(rows) != 1 or key not in jobs:
        raise ValueError('Result key is outside its frozen batch')
    expected, final, _ = item_artifacts(out, directory, jobs[key], rows[0], context)
    for name, value in expected.items():
        if read(local / name) != value:
            raise ValueError('Derived or native result artifact changed: ' + name + ' for ' + key)
    for name in ('final.json', 'error.json', 'decoded-diagnosis.json', 'response.json'):
        if name not in expected and (local / name).exists():
            raise ValueError('Conflicting result artifact: ' + name + ' for ' + key)
    return final


def collect_terminal(out, directory, jobs):
    context = _batch_context(out, directory, jobs)
    ensure(directory / 'billing.json', context['bill'])
    errors, counts, fatal = 0, collections.Counter(), False
    for row in context['envelope']['requests']:
        key = row['custom_id']; local = out / 'results' / key
        files, final, fatal_item = item_artifacts(out, directory, jobs[key], row, context)
        for name in ('final.json', 'error.json', 'decoded-diagnosis.json', 'response.json'):
            if name not in files and (local / name).exists():
                raise ValueError('Conflicting result artifact: ' + name + ' for ' + key)
        for name, value in files.items():
            ensure(local / name, value)
        if final is None:
            fatal = fatal or fatal_item; errors += 1
        else:
            counts[final['status']] += 1
    ensure(directory / 'collected.json', {'jobs': len(context['manifest']['keys']),
        'counts': dict(counts), 'errors': errors, 'terminal_sha256': context['terminal_sha256']})
    emit(out, 'cloud_collected', batch=directory.name, jobs=len(context['manifest']['keys']), counts=dict(counts), errors=errors)
    if directory.name == '000001':
        if errors:
            raise ValueError('Two-card structural canary failed; larger batches remain blocked')
        ensure(out / 'canary-passed.json', {'batch': directory.name, 'keys': context['manifest']['keys'],
            'terminal_sha256': context['terminal_sha256'], 'config_sha256': sha(out / 'config.json')})
    guard_admission(out)
    if fatal or errors >= 10 or (len(context['manifest']['keys']) >= 10 and errors / len(context['manifest']['keys']) >= .1):
        raise ValueError('Fatal or substantial terminal request failures; no further submissions')
...
def report(out, jobs):
    prior = verify_predecessor(out)
    excluded = set(prior['attempted_keys'])
    counts = collections.Counter({s: 0 for s in (*VALID, 'errors', 'prior_errors', 'cloud_pending', 'terminal_failed', 'terminal_awaiting_collection', 'ambiguous_submission', 'unsubmitted')})
    counts['prior_errors'] = prior['technical_errors']
    ...
    for d in batch_dirs(out):
        ...
        status = read(d / 'accepted.json') if (d / 'accepted.json').exists() else {}
        state = batch_state(d)
        if (d / 'submission-intent.json').exists():
            states.update({key: state for key in manifest['keys']})
        batches.append({'batch': d.name, 'jobs': len(manifest['keys']), 'id': status.get('id'),
                       'status': state, 'collected': (d / 'collected.json').exists()})
    with (out / 'review.csv.tmp').open('w') as csvfile, (out / 'diagnoses.jsonl.tmp').open('w') as jsonfile:
        ...
        for key, job in jobs.items():
            if key in excluded:
                if any((out / 'results' / key).glob('*')):
                    raise ValueError('Quarantined predecessor key has new result artifacts')
                continue
            d = out / 'results' / key
            final = read(d / 'final.json') if (d / 'final.json').exists() else None
            if final is not None:
                final = verify_result(out, key, jobs, result_cache)
                ...
                verify_result(out, key, jobs, result_cache)
                counts['errors'] += 1; languages[job['content']['language']]['errors'] += 1
            elif states.get(key) in {'failed', 'cancelled', 'expired'}:
                counts['terminal_failed'] += 1
            elif states.get(key) == 'completed':
                counts['terminal_awaiting_collection'] += 1
            elif states.get(key) == 'ambiguous_submission':
                counts['ambiguous_submission'] += 1
            else:
    ...
    value = {'time': time.time(), 'stage': 'Audit-3', 'mode': 'cloud_batch', 'selected': len(jobs),
        'schema_encoding': SCHEMA_ENCODING, 'eligible_selected': len(jobs) - len(excluded),
        'prior_failed_requests': prior['technical_errors'],
        'prior_unknown_billing_requests': prior['unresolved_requests'],
        'prior_unresolved_requests': prior['unresolved_requests'],
        'prior_unresolved_scope': 'Billing only; all prior request outcomes are confirmed terminal failures.',
        'prior_by_language': prior['by_language'],
        ...
        'terminal_failed_batches': len(terminal_failures(out)),
        'terminal_awaiting_collection_batches': sum(batch_state(d) == 'completed' for d in uncollected_batches(out)),
        ...
        'request_failure_rate': (counts['errors'] + counts['prior_errors'] + counts['terminal_failed']) / (completed + counts['errors'] + counts['prior_errors'] + counts['terminal_failed']) if completed + counts['errors'] + counts['prior_errors'] + counts['terminal_failed'] else None,
        'current_request_failure_rate': (counts['errors'] + counts['terminal_failed']) / (completed + counts['errors'] + counts['terminal_failed']) if completed + counts['errors'] + counts['terminal_failed'] else None,
...
def step(out, jobs, plan, *, collect_only=False, network=B.network, should_stop=lambda: False):
    ...
    errors = []
    for directory in uncollected_batches(out):
...
def main():
    ...
    parser.add_argument('--predecessor-out', type=Path)
    parser.add_argument('--once', action='store_true')
    args = parser.parse_args(); out = args.out.resolve()
    with exclusive_lock(out):
        if args.command == 'prepare':
            if args.jobs is None or args.integration_manifest is None or args.baseline_accounted_usd is None or args.predecessor_out is None:
                parser.error('prepare requires --jobs, --integration-manifest, --baseline-accounted-usd, and --predecessor-out')
            print(json.dumps(prepare(out, args.jobs, args.integration_manifest, args.baseline_accounted_usd, args.predecessor_out)))
