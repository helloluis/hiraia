#!/usr/bin/env python3
"""Audit-3 v5: immutable continuation of verified v4 results.
...
import gemini_audit3_batch_v4 as PREV
...
def predecessor_snapshot(predecessor_out, jobs_path, integration_manifest, baseline):
    ...
    if not seen or PREV.active_batches(prior):
        raise ValueError('Predecessor must have attempts and no active cloud work')
    for directory in PREV.batch_dirs(prior):
        _, envelope = PREV.checked_batch(prior, directory, jobs)
        if not all((directory / name).is_file() for name in ('accepted.json', 'terminal.json', 'billing.json', 'collected.json')):
            raise ValueError('Predecessor batch is not terminal and collected')
        accepted = read(directory / 'accepted.json'); terminal = read(directory / 'terminal.json')
        PREV.validate_remote(accepted, envelope); PREV.validate_remote(terminal, envelope, accepted['id'])
    ...
    files = {prior / name for name in ('config.json', 'plan.json', 'predecessor.json', 'process.json', 'report.json', 'canary-passed.json', 'CANARY-REVIEWED.json')}
...
def verify_predecessor(out):
    ...
    if PREV.active_batches(old_out) or PREV.check_ledger(old_out, old_jobs) != set(prior['attempted_keys']):
        raise ValueError('Predecessor acquired active or differing attempts')
    if [str(path) for path in PREV.batch_dirs(old_out)] != prior['batch_paths']:
        raise ValueError('Predecessor batch inventory changed')
    if PREV.accounting(old_out)['budget_accounted_usd'] != prior['budget_accounted_usd']:
        raise ValueError('Predecessor accounting changed')
    return prior
...
def prepare(out, jobs_path, integration_manifest, baseline_accounted_usd, predecessor_out):
    ...
    config = {'version': 5, 'schema_encoding': SCHEMA_ENCODING, 'diagnosis_encoding': K.ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
    ...
    inherited = {'predecessor_out': str(old), 'predecessor_config_sha256': sha(old / 'config.json'),
        ...
        'predecessor_review_sha256': sha(old / 'CANARY-REVIEWED.json'), 'schema_encoding': SCHEMA_ENCODING,
...
def frozen(out):
    ...
    expected = {'version': 5, 'schema_encoding': SCHEMA_ENCODING, 'diagnosis_encoding': K.ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
