#!/usr/bin/env python3
"""Audit-3 v2: equivalent explicit string enums and quarantined v1 attempts.
...
import gemini_audit3_batch as V1
...
SCHEMA_ENCODING = 'explicit_enum_string_type_v2'
PROMPT = A.e.BASE + '\n' + A.e.DIAG
...
def dependency_hashes():
    ...
    todo, seen, paths = [A, B, C, V1], set(), {Path(__file__).resolve()}
    while todo:
        module = todo.pop()
...
def payload(job):
    value = V1.payload(job)
    schema = value['response_format']['json_schema']['schema']
    value['response_format']['json_schema']['schema'] = explicit_enum_strings(schema)
    return value
...
def predecessor_snapshot(predecessor_out, jobs_path, integration_manifest, baseline):
    """Freeze a stopped, entirely rejected v1 epoch without guessing its charge.
    ...
    if not (prior / 'BLOCKED.json').is_file():
        raise ValueError('Predecessor must be a blocked v1 run')
    ...
    seen = V1.check_ledger(prior, jobs)
    if not seen:
        raise ValueError('Predecessor has no durable attempts to quarantine')
    files = {prior / name for name in ('config.json', 'plan.json', 'BLOCKED.json', 'process.json')}
    ...
    batches = V1.batch_dirs(prior)
    for directory in batches:
        _, envelope = V1.checked_batch(prior, directory, jobs)
        ...
        V1.validate_remote(accepted, envelope)
        terminal = read(directory / 'terminal.json')
        V1.validate_remote(terminal, envelope, accepted['id'])
    ...
    prior_cost = V1.accounting(prior)
    ...
    return {'version': 1, 'out': str(prior), 'runner': str(Path(V1.__file__).resolve()),
        'runner_sha256': sha(V1.__file__),
        ...
        'billing_note': 'Prior failed input returned null usage/results; the full reservation remains unknown, not zero charged.'}
...
def verify_predecessor(out):
    ...
    if [str(path) for path in V1.batch_dirs(old_out)] != prior['batch_paths']:
    ...
    for directory in V1.batch_dirs(old_out):
...
def prepare(out, jobs_path, integration_manifest, baseline_accounted_usd, predecessor_out):
    ...
    config = {'version': 2, 'schema_encoding': SCHEMA_ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
...
def frozen(out):
    config = read(out / 'config.json')
    expected = {'version': 2, 'schema_encoding': SCHEMA_ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
    ...
    if any(config.get(k) != v or type(config.get(k)) is not type(v) for k, v in expected.items()):
        raise ValueError('Frozen configuration does not match authorized Audit-3 v2 settings')
