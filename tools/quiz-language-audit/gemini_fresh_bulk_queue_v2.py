...
immutable. Bulk records live under bulk-v2/. This coordinator holds Fresh-1's
...
class BudgetCapacity(ValueError):
    """Wait for existing reservations to settle, or stop if none can settle."""
...
def batch_dirs(out):
    return sorted((out / 'bulk-v2/batches').glob('[0-9]*'))
...
def canary_release_ready(out):
    ...
    passed_path = out / 'bulk-v2/canary-passed.json'
    if not passed_path.exists():
        return True
    release_path = out / 'bulk-v2/CANARY-REVIEWED.json'
...
def baseline_state(out):
    config = T.read(out / 'bulk-v2/config.json')
...
def report(out, entries):
    ...
    bulk = {'time': time.time(), 'mode': 'cloud_batch_v2', 'batch_size': 500,
        'maximum_outstanding_batches': 4, 'poll_seconds': 60,
    ...
    summary = {'time': time.time(), 'selected': len(entries), 'counts': dict(counts),
        ...
        'bulk-v2': bulk, 'known_source_blocked_keys': len(blocked),
    ...
    write(out / 'bulk-v2/summary.json', bulk)
...
def prepare(out):
    ...
    received, reserved = 0.0, 0.0
    for item in entries:
        directory = T.directory(out, item)
        if T.attempted(out, item) and not any((directory / name).exists() for name in ('final.json', 'error.json')):
            raise ValueError('Reconcile every earlier attempt before version2 preparation')
        charge = C.charge_state(directory)
        received += charge[0]
        reserved += charge[1]
    baseline = {'received_cost_usd': received, 'uncertain_and_active_reserved_usd': reserved,
        'note': 'Verified all pre-v2 card charges exactly once, including zero-cost v1 failures.'}
    config = {'time': time.time(), 'mode': 'cloud_batch_v2', 'original_config_sha256': sha(out / 'config.json'),
        'prompt_sha256': T.digest(prompt), 'jobs_sha256': sha(Path(legacy['jobs'])),
        'model': T.MODEL, 'canonical_model': B.RESOLVED_MODEL,
        'batch_size': 500, 'maximum_outstanding_batches': 4, 'poll_seconds': 60,
        'budget_usd': 30, 'combined_budget_usd': 250, 'canary_size': 2,
        'canary_handover_gate': 'bulk-v2/CANARY-REVIEWED.json matching terminal hash and verified Audit-2 selection',
        ...
        'dependencies_sha256': {str(Path(p).resolve()): sha(p)
            for p in (__file__, C.__file__, C.V1.__file__, Q.__file__, T.__file__, B.__file__, T.rewrite.__file__, T.flex.__file__)}}
    ...
    write(bulk / 'authorization.json', {'time': time.time(),
        'user_request': 'Ok please fix the schema so we can proceed with the batches; user clarified that500 is upload size, not a one-outstanding-batch limit.',
        'scope': 'Version2 nullable-object wire schema; two untouched probe jobs reused once, then budget-reserved uploads up to500 each, at most4 outstanding provider jobs, automatic collection and Audit-2 feeding. No retry of old failed IDs.',
...
def frozen_entries(out):
    ...
    changed = {p for p, digest in config['dependencies_sha256'].items() if sha(Path(p)) != digest}
    if changed:
        validate_polling_recovery(out, config, changed)
    entries = Q.load_entries(out)
    by_id = {i['sample_id']: i for i in entries}
    for item in T.read(out / 'bulk-v2/plan.json'):
...
def validate_polling_recovery(out, config, changed=None):
    """Bind one reviewed status-visibility recovery; preserve the original freeze."""
    root = out / 'bulk-v2/recovery'
    record = T.read(root / 'authorization.json')
    runner = str(Path(__file__).resolve())
    changed = changed if changed is not None else {
        p for p, digest in config['dependencies_sha256'].items() if sha(Path(p)) != digest}
    if (changed != {runner} or record.get('kind') != 'accepted_batch_read_visibility'
            or record.get('config_sha256') != sha(out / 'bulk-v2/config.json')
            or record.get('blocked_sha256') != sha(out / 'bulk-v2/BLOCKED.json')
            or record.get('old_runner_sha256') != config['dependencies_sha256'][runner]
            or sha(root / 'runner-before-recovery.py') != record['old_runner_sha256']
            or record.get('new_runner_sha256') != sha(Path(__file__))
            or record.get('status_sha256') != sha(root / 'visible-batch-status.json')):
        raise ValueError('Unverified polling recovery or unrelated frozen dependency change')
    status = T.read(root / 'visible-batch-status.json')
    blocked = T.read(out / 'bulk-v2/BLOCKED.json')
    batch_id = record.get('batch_id')
    accepted = T.read(out / 'bulk-v2/batches/000001/accepted.json')
    if (status.get('id') != batch_id or accepted.get('id') != batch_id
            or status.get('model') != accepted.get('model')
            or status.get('endpoint') != accepted.get('endpoint')
            or status.get('request_counts', {}).get('total') != accepted['request_counts']['total']
            or status.get('status') not in ('validating', 'in_progress', 'completed')
            or 'HTTP 404:' not in blocked.get('reason', '') or batch_id not in blocked['reason']):
        raise ValueError('Recovery does not prove the same accepted batch became visible')
    return record


def next_items(out, by_id, prompt):
    blocked = T.read(out / 'source-blocks.json')
    unused = [by_id[p['sample_id']] for p in T.read(out / 'bulk-v2/plan.json')
    ...
    limit = 500 if (out / 'bulk-v2/canary-passed.json').exists() else 2
    ...
    for item in same_schema[:limit]:
        ...
        result.append(item)
    if not result:
        raise BudgetCapacity('Existing budget cannot reserve the next cloud request')
    return result


def fill_window(out, by_id, prompt, config, remaining_batches=None, should_stop=lambda: False):
    """Admit independent uploads until the monetary or provider window fills."""
    if not canary_release_ready(out):
        return [], False, 'canary_handover'
    canary_passed = (out / 'bulk-v2/canary-passed.json').exists()
    if not canary_passed and batch_dirs(out):
        return [], False, 'canary_result'
    added = []
    while len(active_batches(out)) < config['maximum_outstanding_batches']:
        if should_stop():
            return added, False, 'signal'
        if (out / 'bulk-v2/STOP').exists():
            return added, False, 'STOP sentinel'
        if remaining_batches is not None and len(added) >= remaining_batches:
            return added, False, 'requested batch limit'
        try:
            items = next_items(out, by_id, prompt)
        except BudgetCapacity:
            if active_batches(out):
                return added, False, 'budget_reserved_for_outstanding_work'
            raise
        if not items:
            return added, True, None
        added.append(submit(out, items, prompt))
        if not canary_passed:
            return added, False, 'canary_result'
    return added, False, 'outstanding_batch_limit'


def submit(out, items, prompt):
    number = len(batch_dirs(out)) + 1
    directory = out / 'bulk-v2/batches' / f'{number:06d}'
    ...
    manifest = {'number': number, 'sample_ids': [i['sample_id'] for i in items],
        ...
        'schema_version': 2, 'schema_sha256': T.digest(requests[0]['body']['response_format']), 'created': time.time()}
...
def reconcile_zero_failure(out, directory, by_id):
    """Record verified terminal failures without changing raw provider evidence."""
    terminal = T.read(directory / 'terminal.json')
    requests = T.read(directory / 'request.json')['requests']
    C.reconcile_failed_zero_cost(terminal, requests)
    ensure(directory / 'billing.json', {'received_cost_usd': 0,
        'usage': terminal['usage'], 'terminal_sha256': sha(directory / 'terminal.json')})
    records = {r['custom_id']: r for r in terminal['results']}
    for request in requests:
        number = int(request['custom_id']); item = by_id[number]
        result = T.directory(out, item)
        receipt = C.make_failure_receipt(T.read(result / 'batch-origin.json'))
        ensure(result / 'batch-failure-receipt.json', receipt)
        ensure(result / 'error.json', {'time': terminal['finalized_at'],
            'kind': 'cloud_batch_request', 'provider_detail': records[str(number)]['error'],
            'fatal': False, 'automatic_retry': False})
        ensure(out / 'outcomes' / f'{number}.json', {'time': terminal['finalized_at'],
            'sample_id': number, 'key': item['job']['key'], 'status': 'error',
            'fatal': False, 'received_cost_usd': 0, 'uncertain_reserved_usd': 0,
            'billing_scope': 'verified_zero_usage_terminal_failure'})
    ensure(directory / 'collected.json', {'time': terminal['finalized_at'],
        'jobs': len(requests), 'failures': len(requests), 'zero_cost_failure_reconciled': True})
...
def collect(out, directory, by_id):
    ...
    if terminal.get('error'):
        reconcile_zero_failure(out, directory, by_id)
        raise ValueError('All cloud requests failed upstream; terminal zero cost reconciled, stop submissions')
    ...
    if not (out / 'bulk-v2/canary-passed.json').exists():
        if failed or not proposals:
            raise ValueError('Cloud-format canary failed; do not submit larger batches')
        write(out / 'bulk-v2/canary-passed.json', {'time': time.time(), 'batch': int(directory.name),
...
def run(out, max_batches=None, resume_known_batches=False):
    config, entries, by_id, prompt = frozen_entries(out)
    recovery = out / 'bulk-v2/recovery'
    if (out / 'bulk-v2/STOP').exists():
        raise ValueError('Bulk STOP sentinel is present')
    if resume_known_batches:
        validate_polling_recovery(out, config)
        if (recovery / 'consumed.json').exists() or (recovery / 'BLOCKED.json').exists():
            raise ValueError('Polling recovery was already used; no automatic restart')
        for directory in active_batches(out):
            if directory.name != '000001' or not (directory / 'accepted.json').exists():
                raise ValueError('Unexpected outstanding submission during controlled recovery')
        write(recovery / 'consumed.json', {'time': time.time(), 'pid': os.getpid()}, immutable=True)
    elif (out / 'bulk-v2/BLOCKED.json').exists():
        raise ValueError('A previous bulk failure requires explicit reconciliation before more spending')
    if active_batches(out) and not resume_known_batches:
        raise ValueError('Existing unfinished batch requires collection/reconciliation, never blind relaunch')
    process = {'pid': os.getpid(), 'started': time.time(), 'status': 'running',
        'mode': 'cloud_batch_v2', 'bulk_dir': 'bulk-v2', 'argv': sys.argv,
    ...
    submitted = 0
    due = {str(directory): time.monotonic() for directory in active_batches(out)}
    try:
        while not reason:
            # Poll every outstanding ID; the oldest slow job never blocks admission
            # or collection of another completed batch.
            for directory in active_batches(out):
                if reason:
                    break
                if time.monotonic() < due.get(str(directory), 0):
                    continue
                accepted = T.read(directory / 'accepted.json')
                _, status = B.network(B.API + '/' + accepted['id'])
                if status.get('id') != accepted['id']:
                    raise ValueError('Provider batch ID changed')
                write(directory / 'status.json', status)
                emit('cloud_poll', batch=int(directory.name), id=status['id'],
                     status=status.get('status'), request_counts=status.get('request_counts'))
                due[str(directory)] = time.monotonic() + config['poll_seconds']
                if status.get('status') in TERMINAL:
                    ensure(directory / 'terminal.json', status)
                    collect(out, directory, by_id)
            if reason:
                break
            stop_requested = (out / 'bulk-v2/STOP').exists()
            limit_reached = max_batches is not None and submitted >= max_batches
            exhausted, waiting = False, None
            if not stop_requested and not limit_reached:
                added, exhausted, waiting = fill_window(out, by_id, prompt, config,
                    None if max_batches is None else max_batches - submitted,
                    should_stop=lambda: bool(reason))
                submitted += len(added)
                for directory in added:
                    # Acceptance can precede status visibility on this provider.
                    due[str(directory)] = time.monotonic() + config['poll_seconds']
            outstanding = active_batches(out)
            report(out, entries)
            emit('cloud_window', outstanding_batches=len(outstanding), waiting=waiting,
                 max_outstanding_batches=config['maximum_outstanding_batches'])
            if not outstanding and (stop_requested or limit_reached or exhausted):
                if stop_requested or limit_reached:
                    reason.append('STOP sentinel' if stop_requested else 'requested batch limit')
                break
            # Allow newly accepted jobs to become visible; publish budget snapshots
            # and recheck the canary handover / STOP at most every sixty seconds.
            delay = min([config['poll_seconds']] +
                        [max(0, due.get(str(d), 0) - time.monotonic()) for d in outstanding])
            deadline = time.monotonic() + max(0.1, delay)
            while not reason and time.monotonic() < deadline:
                time.sleep(min(1, max(0, deadline - time.monotonic())))
        process.update(status='stopped' if reason else 'finished',
                       reason=reason[0] if reason else None)
    except BaseException as error:
        process.update(status='stopped', reason=type(error).__name__ + ': ' + str(error))
        block_path = recovery / 'BLOCKED.json' if resume_known_batches else out / 'bulk-v2/BLOCKED.json'
        ensure(block_path, {'time': time.time(), 'reason': process['reason'],
...
def main():
    ...
    parser.add_argument('--resume-known-batches', action='store_true')
    ...
    with Q.exclusive_lock(out):
        if args.command == 'prepare':
            ...
            run(out, args.max_batches, args.resume_known_batches)
