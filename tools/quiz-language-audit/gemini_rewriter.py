"""Bounded Gemini Flex rewrite trials. Outputs are proposals; never applies edits.
...
write = qwen.E.a.write_json
canonical = qwen.E.a.canonical
digest = qwen.E.a.digest
...
def payload(item, prompt):
    ...
    schema = qwen.E.schema(content, 'rewrite')
    data = {key: content[key] for key in ('english', 'target', 'grades', 'language')}
    data['language'] = qwen.E.a.LANGUAGES[content['language']]
    data['source_context_english'] = content.get('source_fact', {}).get('en')
    data['audit'] = item['audit']['diagnosis']
    return {'model': MODEL, 'temperature': 0, 'max_tokens': 8000,
        ...
        'response_format': {'type': 'json_schema', 'json_schema': {
            'name': 'rewrite', 'strict': True, 'schema': schema}},
        'messages': [{'role': 'system', 'content': prompt}, {'role': 'user',
            'content': canonical({'quiz': data, 'response_schema': schema})}]}
...
def validate(raw, item):
    ...
    changes = qwen.validate(result, item)
    if not result['note'].strip():
        raise ValueError('Empty rewrite note')
    for change in changes:
        if not any(character.isalnum() for character in change['after']):
            raise ValueError('Punctuation-only edited field: ' + change['field'])
    if result['proposed']:
        original = item['job']['content']['target']['options']
        proposed = result['proposed']['options']
        for index, value in enumerate(proposed):
            if value != original[index] and value in original:
                raise ValueError('Option moved from its original position')
    return {'status': 'held' if result['status'] == 'hold' else 'proposal',
...
def validate_sample(items):
    ...
    for item in items:
        ...
        diagnosis = item['audit']
        checked = flex.audit.validate({'choices': [{'finish_reason': 'stop', 'message': {
            'content': canonical(diagnosis['diagnosis'])}}]}, content)
        if (diagnosis['status'] != 'flagged' or diagnosis.get('answer_matches') is not True
                or checked['status'] != 'flagged' or not checked['answer_matches']
                or diagnosis['diagnosis']['whole_quiz']['status'] != 'ok'):
            raise ValueError('Only confirmed, answer-matching, source-safe flagged jobs are eligible')
        keys.add(key)
        ids.add(number)
...
def prepare(args, items, prompt):
    ...
    if not prompt.strip():
        raise ValueError('Empty editing prompt')
    ...
    dependencies = (Path(__file__), Path(qwen.__file__), Path(flex.__file__),
                    Path(flex.audit.__file__), Path(qwen.E.__file__))
    config = {'model': MODEL, 'service_tier': 'flex', 'sample_sha256': digest(items),
        ...
        'total_conservative_reservations_usd': sum(ceilings.values()), 'diagnosis_guided_proposals_only': True}
...
def report(out, items):
    ...
    records, lines = [], ['# Gemini Flex rewrite trial', '',
    ...
    for item in items:
        ...
        record = {'sample_id': item['sample_id'], 'key': item['job']['key'],
            ...
            'before': content['target'], 'audit': item['audit'], **outcome}
        ...
        lines += [f'## {item["sample_id"]}. {content["language"]}: {content["english"]["q"]}', '',
            ...
            'Before:', '```json', json.dumps(content['target'], ensure_ascii=False, indent=2), '```', '',
            'Audit evidence:', '```json', json.dumps(item['audit']['diagnosis'], ensure_ascii=False, indent=2), '```', '',
...
def credential():
    ...
    if not value:
        for line in (flex.audit.REPO / '.env.local').read_text().splitlines():
...
def main():
    ...
    with (args.out / 'run.lock').open('a') as lock:
        ...
        process = {'pid': os.getpid(), 'started': time.time(), 'status': 'running',
            'command': 'gemini_rewriter.py run', 'argv': sys.argv,
