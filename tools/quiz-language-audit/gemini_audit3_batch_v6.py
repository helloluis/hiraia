#!/usr/bin/env python3
"""Audit-3 v6: concurrent extension of v5 with a separate authorized budget.
...
import gemini_audit3_batch_v5 as PREV
...
def payload(job):
    # V4 already applies the approved reversible wire encoding. Reapplying it
    # would transform an established option_ key a second time.
...
def predecessor_snapshot(predecessor_out, jobs_path, integration_manifest, baseline):
    """Freeze a fully collected v4 epoch as immutable continuation history."""
    prior = Path(predecessor_out).resolve()
    process = read(prior / 'process.json')
    if process.get('status') not in ('stopped', 'finished', 'running') or (process.get('status') == 'running' and not (prior / 'STOP').exists()):
        raise ValueError('Predecessor must be stopped or have a durable STOP admission hold')
    ...
    if not seen:
        raise ValueError('Predecessor must have durable attempts')
    for directory in PREV.batch_dirs(prior):
        _, envelope = PREV.checked_batch(prior, directory, jobs)
        if not (directory / 'accepted.json').is_file():
            raise ValueError('Predecessor batch lacks a durable accepted ID')
        accepted = read(directory / 'accepted.json')
        PREV.validate_remote(accepted, envelope)
        if (directory / 'terminal.json').is_file():
            if not all((directory / name).is_file() for name in ('billing.json', 'collected.json')):
                raise ValueError('Terminal predecessor batch is not collected')
            PREV.validate_remote(read(directory / 'terminal.json'), envelope, accepted['id'])
        elif not (directory / 'submission-intent.json').is_file():
            raise ValueError('Live predecessor batch lacks its durable intent')
    if not PREV.canary_ready(prior, jobs):
        raise ValueError('Predecessor canary approval is not verified')
    prior_cost = PREV.accounting(prior)
    files = {prior / name for name in ('config.json', 'plan.json', 'predecessor.json', 'CANARY-REUSED.json', 'STOP')}
    files.update(Path(path) for path in config['dependencies_sha256'])
    files.update((jobs_path, integration_manifest))
    historical = sorted({str(prior), *read(prior / 'predecessor.json')['historical_outputs']})
    return {'version': 4, 'out': str(prior), 'historical_outputs': historical,
        'files_sha256': {str(path): sha(path) for path in sorted(files)},
        'batch_paths': [str(path) for path in PREV.batch_dirs(prior)], 'attempted_keys': sorted(seen),
        'technical_errors': sum(1 for key in seen if (prior / 'results' / key / 'error.json').exists()),
        'unresolved_requests': 0, 'by_language': {lang: sum(1 for key in seen if jobs[key]['content']['language'] == lang) for lang in ('tl', 'bis')},
        'received_cost_usd': prior_cost['received_cost_usd'], 'received_usage': prior_cost['received_usage'],
        'uncertain_charge_reserved_usd': prior_cost['uncertain_charge_reserved_usd'],
        'active_reserved_usd': prior_cost['active_reserved_usd'], 'budget_accounted_usd': prior_cost['budget_accounted_usd'],
        'billing_note': 'V4 aggregate bills and prior unresolved exposure are retained once; v5 excludes every prior attempt.'}
...
def verify_predecessor(out):
    config = read(out / 'config.json'); path = out / 'predecessor.json'
    if sha(path) != config['predecessor_sha256']:
        raise ValueError('Frozen predecessor record changed')
    prior = read(path)
    if prior['out'] != config['predecessor_out'] or prior['attempted_keys'] != config['excluded_keys'] or prior['budget_accounted_usd'] != config['predecessor_accounted_usd']:
        raise ValueError('Frozen predecessor configuration differs')
    for name, expected in prior['files_sha256'].items():
        if sha(Path(name)) != expected:
            raise ValueError('Frozen predecessor artifact changed: ' + name)
    old_out = Path(prior['out']); old_config, old_jobs, _ = PREV.frozen(old_out)
    if not (old_out / 'STOP').exists() or PREV.check_ledger(old_out, old_jobs) != set(prior['attempted_keys']):
        raise ValueError('Predecessor admission hold or attempt set changed')
    if [str(path) for path in PREV.batch_dirs(old_out)] != prior['batch_paths']:
        raise ValueError('Predecessor batch inventory changed')
    return prior
...
def prepare(out, jobs_path, integration_manifest, baseline_accounted_usd, predecessor_out):
    ...
    config = {'version': 6, 'schema_encoding': SCHEMA_ENCODING, 'diagnosis_encoding': K.ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
    ...
    old = predecessor_out
    inherited = {'predecessor_out': str(old), 'predecessor_config_sha256': sha(old / 'config.json'),
        'predecessor_canary_terminal_sha256': sha(old / 'batches' / '000001' / 'terminal.json'),
        'predecessor_review_sha256': sha(old / 'CANARY-REUSED.json'), 'schema_encoding': SCHEMA_ENCODING,
        'reason': 'Same verified v4 wire schema; continuation excludes prior attempts and does not repeat a paid probe.'}
    ensure(out / 'CANARY-REUSED.json', inherited)
    return config


def frozen(out):
    config = read(out / 'config.json')
    expected = {'version': 6, 'schema_encoding': SCHEMA_ENCODING, 'diagnosis_encoding': K.ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
...
def canary_ready(out, jobs=None):
    inherited = out / 'CANARY-REUSED.json'
    if inherited.exists():
        proof = read(inherited); prior = verify_predecessor(out); old = Path(prior['out'])
        expected = {'predecessor_out': str(old), 'predecessor_config_sha256': sha(old / 'config.json'),
            'predecessor_canary_terminal_sha256': sha(old / 'batches' / '000001' / 'terminal.json'),
            'predecessor_review_sha256': sha(old / 'CANARY-REUSED.json'), 'schema_encoding': SCHEMA_ENCODING,
            'reason': 'Same verified v4 wire schema; continuation excludes prior attempts and does not repeat a paid probe.'}
        if proof != expected or not PREV.canary_ready(old):
            raise ValueError('Inherited v4 canary proof differs from verified predecessor')
        return True
...
def next_keys(out, jobs, plan):
    ...
    if not pending:
        return []
    canary = not batch_dirs(out) and not (out / 'CANARY-REUSED.json').exists()
...
def submit(out, jobs, keys, network=B.network):
    ...
    if not batch_dirs(out) and not (out / 'CANARY-REUSED.json').exists() and len(keys) != CANARY_SIZE:
...
def collect_terminal(out, directory, jobs):
    ...
    if directory.name == '000001' and not (out / 'CANARY-REUSED.json').exists():
        if errors:
