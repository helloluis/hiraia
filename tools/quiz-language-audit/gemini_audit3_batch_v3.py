#!/usr/bin/env python3
"""Audit-3 v3: explicit strict wire format and quarantined v1/v2 attempts.
...
import gemini_audit3_batch_v2 as PREV
...
SCHEMA_ENCODING = 'explicit_enum_string_type_strict_v3'
PROMPT = A.e.BASE + '\n' + A.e.DIAG
...
def dependency_hashes():
    ...
    todo, seen, paths = [A, B, C, PREV], set(), {Path(__file__).resolve()}
    while todo:
        module = todo.pop()
...
def payload(job):
    # Complete the provider's wire protocol without altering the schema, prompt,
    # target, private answer handling or original diagnosis validation.
    value = PREV.payload(job)
...
def predecessor_snapshot(predecessor_out, jobs_path, integration_manifest, baseline):
    """Freeze a stopped, entirely rejected v2 epoch and its complete v1 history.
    ...
    if not (prior / 'BLOCKED.json').is_file():
        raise ValueError('Predecessor must be a blocked v2 run')
    ...
    if not batches:
        raise ValueError('Predecessor has no v2 attempted batch')
    ...
    return {'version': 2, 'out': str(prior), 'ancestor_out': ancestor['out'],
        ...
        'billing_note': 'Both prior epochs returned null usage/results; their cumulative reservation remains unknown, not zero charged.'}
...
def prepare(out, jobs_path, integration_manifest, baseline_accounted_usd, predecessor_out):
    ...
    ancestor_out = Path(prior['ancestor_out'])
    if out.resolve().is_relative_to(ancestor_out) or ancestor_out.is_relative_to(out.resolve()):
    ...
    config = {'version': 3, 'schema_encoding': SCHEMA_ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
...
def frozen(out):
    config = read(out / 'config.json')
    expected = {'version': 3, 'schema_encoding': SCHEMA_ENCODING, 'stage': 'Audit-3', 'mode': 'cloud_batch',
    ...
    if any(config.get(k) != v or type(config.get(k)) is not type(v) for k, v in expected.items()):
        raise ValueError('Frozen configuration does not match authorized Audit-3 v3 settings')
...
def report(out, jobs):
    ...
    counts = collections.Counter({s: 0 for s in (*VALID, 'errors', 'prior_errors', 'cloud_pending', 'unsubmitted')})
    ...
    value = {'time': time.time(), 'stage': 'Audit-3', 'mode': 'cloud_batch', 'selected': len(jobs),
        ...
        'prior_failed_requests': prior['technical_errors'], 'prior_unresolved_requests': prior['unresolved_requests'],
        ...
        'request_failure_rate': (counts['errors'] + counts['prior_errors']) / (completed + counts['errors'] + counts['prior_errors']) if completed + counts['errors'] + counts['prior_errors'] else None,
        'current_request_failure_rate': counts['errors'] / (completed + counts['errors']) if completed + counts['errors'] else None,
