#!/usr/bin/env bash
# ============================================================================
# deploy_v8_distill.sh — provision a GPU pod, upload the v8 dataset+script, and launch the
# skill-only v8 LoRA train DETACHED. Prints monitor/download/terminate commands (no auto-terminate
# — you control billing). v8 = v6 trainset + a TARGETED safety/myth patch (confident yes/no safety +
...
[ -f distill/train-distill-v8.jsonl ] || { echo "ERR: distill/train-distill-v8.jsonl missing"; exit 1; }
[ -f train-distill-v8.py ] || { echo "ERR: train-distill-v8.py missing"; exit 1; }
...
if [ -z "$POD_ID" ]; then
  ...
  for CLOUD in SECURE COMMUNITY; do for G in "${GPUS[@]}"; do
    ...
    # which the SSH wait below can never reach (observed 3x on 2026-06-15 v8 deploys).
...
scp $SSH -P "$PORT" distill/train-distill-v8.jsonl "root@$IP:/workspace/train-distill-v8.jsonl"
scp $SSH -P "$PORT" train-distill-v8.py "root@$IP:/workspace/train-distill-v8.py"

echo ">> launching v8 train detached ..."
ssh $SSH -p "$PORT" "root@$IP" \
  'cd /workspace && rm -f train-v8.log && source venv/bin/activate && setsid nohup python train-distill-v8.py > train-v8.log 2>&1 < /dev/null & echo "launched pid $!"'

cat <<EOF

================= v8 TRAIN LIVE: $POD_ID  (root@$IP:$PORT) =================
Watch:    ssh $SSH -p $PORT root@$IP 'tail -f /workspace/train-v8.log'
Download when done (look for "Adapter saved"):
  scp $SSH -P $PORT -r root@$IP:/workspace/output/distill-sailor-3b-v8/final-adapter adapters/distill-sailor-3b-v8/
