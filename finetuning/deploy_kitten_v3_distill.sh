#!/usr/bin/env bash
# ============================================================================
# deploy_v9_distill.sh — provision a GPU pod, upload the v9 dataset+script, and launch the
# skill-only v9 LoRA train DETACHED. Prints monitor/download/terminate commands (no auto-terminate
# — you control billing). v9 = v6 trainset + a TARGETED safety/myth patch (confident yes/no safety +
...
[ -f distill/train-distill-kitten-v3.jsonl ] || { echo "ERR: distill/train-distill-kitten-v3.jsonl missing"; exit 1; }
[ -f train-distill-kitten.py ] || { echo "ERR: train-distill-kitten.py missing"; exit 1; }
...
# SKIP_VOLUME=1 forces the fresh-venv throwaway path — the volume's cached venv hung on import
# (torch/CUDA mismatch) on 2026-06-16, so a fresh install is more reliable.
[ "${SKIP_VOLUME:-0}" = 1 ] && echo ">> SKIP_VOLUME=1 — skipping persistent-volume path (fresh venv)"
for G in "${GPUS[@]}"; do
  [ "${SKIP_VOLUME:-0}" = 1 ] && break
...
if [ -z "$POD_ID" ]; then
  ...
  for CLOUD in SECURE COMMUNITY; do for G in "${GPUS[@]}"; do
    ...
    # which the SSH wait below can never reach (observed 3x on 2026-06-15 v9 deploys).
...
scp $SSH -P "$PORT" distill/train-distill-kitten-v3.jsonl "root@$IP:/workspace/train-distill-v7.jsonl"
scp $SSH -P "$PORT" train-distill-kitten.py "root@$IP:/workspace/train-distill-kitten.py"

echo ">> launching kitten train detached ..."
ssh $SSH -p "$PORT" "root@$IP" \
  'cd /workspace && rm -f train-kitten.log KITTEN_DONE KITTEN_FAILED && source venv/bin/activate && setsid nohup bash -c "python train-distill-kitten.py > train-kitten.log 2>&1; if [ -d /workspace/output/distill-sailor-1b-kitten/final-adapter ]; then touch /workspace/KITTEN_DONE; else touch /workspace/KITTEN_FAILED; fi" < /dev/null & echo "launched pid $!"'

# UNATTENDED BILLING SAFETY: a pod-side watchdog that self-terminates after 5h no matter what,
# so even if the laptop/agent monitoring drops, the pod can't bill indefinitely. The primary
# stop is the explicit terminate after we download; this is the backstop.
echo ">> arming 5h self-terminate watchdog (billing safety) ..."
# Generate the watchdog locally with a QUOTED heredoc (literal — keeps the \" the GraphQL-in-JSON
# needs) then sed-substitute the secrets, scp it, and launch detached. Avoids nested-quote breakage.
cat > /tmp/kitten-watchdog.sh <<'WDEOF'
#!/bin/bash
sleep 18000
curl -s "https://api.runpod.io/graphql?api_key=__KEY__" -H 'Content-Type: application/json' -d '{"query":"mutation { podTerminate(input: {podId: \"__POD__\"}) }"}'
WDEOF
sed -i '' -e "s|__KEY__|${RUNPOD_API_KEY}|" -e "s|__POD__|${POD_ID}|" /tmp/kitten-watchdog.sh
scp $SSH -P "$PORT" /tmp/kitten-watchdog.sh "root@$IP:/workspace/kitten-watchdog.sh"
ssh $SSH -p "$PORT" "root@$IP" 'setsid nohup bash /workspace/kitten-watchdog.sh >/dev/null 2>&1 < /dev/null & echo "watchdog armed (5h)"'

cat <<EOF

================= KITTEN TRAIN LIVE: $POD_ID  (root@$IP:$PORT) =================
Watch:    ssh $SSH -p $PORT root@$IP 'tail -f /workspace/train-kitten.log'
Download when done (look for "Adapter saved"):
  scp $SSH -P $PORT -r root@$IP:/workspace/output/distill-sailor-1b-kitten/final-adapter adapters/distill-sailor-1b-kitten-v3/
