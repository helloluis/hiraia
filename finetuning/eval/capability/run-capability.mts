...
// `answer` is the DISPLAYED text (image tags stripped) — what the judge scores. `raw` keeps the
// model's literal output (tags intact) so the objective presentation metrics (illustration
// restraint, emoji, bold — see ../presentation.mts) can be measured.
interface AnswerRow { probe: Probe; answer: string; retrievedIds: string[]; samples: string[]; raw?: string }
...
// embeds a contextual query as-is (LocalEngine.ragSearch's R2 path).
...
async function collectAnswers(): Promise<AnswerRow[]> {
  ...
  //    EVERY probe is single-turn: the product is a single-turn card writer, so there is no
  //    dialogue runner here any more (it went with the 'multi-turn' tier).
...
function report(rows: ScoreRow[], baseline?: ScoreRow[]) {
  ...
  // emission + well-formedness) and engagement (emoji presence/spam, bold).
  ...
  const presStats = (rs: ScoreRow[]) => {
    ...
    return { n: rs.length, withTag, wellFormed, emojiPresent, emojiSpam, bold };
  };
  ...
  console.log(`\n  presentation (objective — v3 targets; ${ps.n} probes):`);
