...
# raw results to <repo>/eval/routing-<label>.json, self-terminates. A failure HOLDS the pod.
...
echo "=== ROUTING BENCH $LABEL  $REPO  samples=$SAMPLES T=$TEMP  $(date -u +%FT%TZ) ==="
...
def ask(mode,user):
    r=requests.post("http://localhost:8080/v1/chat/completions",timeout=300,json={
        ...
        "max_tokens":220,"temperature":TEMP,"chat_template_kwargs":{"enable_thinking":False}})
...
print("DONE", {m:f"{sum(v)}/{len(v)}" for m,v in by.items()}, flush=True)
P
TOTAL=$(python3 -c "import json;print(len(json.load(open('/root/probes.json')))*$SAMPLES)")
( while sleep 45; do N=$(grep -a "\[prog\]" $LOG | tail -1 | grep -oE "^\[prog\] [0-9]+" | grep -oE "[0-9]+$"); hb "${N:-0}" "$TOTAL" eval "routing $LABEL: ${N:-0}/$TOTAL completions"; done ) & SIDE=$!
hb 0 "$TOTAL" eval "routing $LABEL: running $TOTAL completions (x$SAMPLES, T=$TEMP)"
...
HfApi(token=os.environ["HF_TOKEN"]).upload_file(path_or_fileobj="/root/routing-$LABEL.json",path_in_repo="eval/routing-$LABEL.json",repo_id="$REPO",commit_message="template-routing benchmark $LABEL (x$SAMPLES, T=$TEMP, fastText lid.176)")
print("UPLOADED")
P
SCORE=$(grep -a "^DONE" $LOG | tail -1 | cut -c6-)
hb 0 0 done "ROUTING $LABEL COMPLETE — $SCORE"
