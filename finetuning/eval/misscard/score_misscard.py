...
PREAMBLE=0; TRUNC=0
def clean(l):
    l=re.sub(r"\*\*|__|`","",l)                       # markdown emphasis
    l=re.sub(r"^\s*(?:[-•*]|\d+[.)])\s*","",l)        # bullets / numbering
    return l.strip()
def directions(ans):
    ...
    if lines and (lines[0].endswith(":") or (len(lines)>1 and not re.search(r"\s[-–—:]\s",lines[0]) and len(lines[0])<60)):
        PREAMBLE+=1; lines=lines[1:]
    lines=[l for l in lines if len(l)>3 and not l.endswith(":")]
    if lines and (len(lines[-1])<12 or not re.search(r"[.!?)\"]\s*$",lines[-1])): TRUNC+=1; lines=lines[:-1]   # "3. K" / mid-sentence cut
    return lines
def topic_of(x):
    t=re.split(r"\s+[-–—:]\s+",x,maxsplit=1)[0].strip(" .:")
    return t
...
for r in rs:
    ...
    topics=[topic_of(x) for x in ds[:3]]
...
print(f"{'mode':10}{'n':>5}{'>=3 dirs':>10}{'mode lang':>11}{'first 3 retrievable':>21}")
...
print(f"  preamble stripped: {PREAMBLE}/{tot}   truncated last line (max_tokens too low): {TRUNC}/{tot}")
print("\nsample outputs:")
