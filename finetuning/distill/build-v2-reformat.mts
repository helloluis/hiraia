...
const out: string[] = [];
let missing = 0;
for (const line of readFileSync('finetuning/distill/dataset-v1.jsonl', 'utf8').split('\n')) {
  if (!line.trim()) continue;
...
writeFileSync('finetuning/distill/v2-correct.jsonl', out.join('\n') + '\n');
console.log(`reformatted ${out.length} correct-fact rows (missing facts: ${missing}) -> v2-correct.jsonl`);
