...
DISK_GB=60; POD_NAME="hiraia-kitten-dpo"
...
[ -f distill/train-distill-kitten-v7.jsonl ] || { echo "ERR: distill/train-distill-kitten-v7.jsonl missing"; exit 1; }
[ -f distill/train-dpo-kitten-safety.jsonl ] || { echo "ERR: distill/train-dpo-kitten-safety.jsonl missing"; exit 1; }
[ -f train-distill-kitten-v7.py ] || { echo "ERR: train-distill-kitten-v7.py missing"; exit 1; }
[ -f train-dpo-kitten.py ] || { echo "ERR: train-dpo-kitten.py missing"; exit 1; }
...
echo ">> uploading datasets + train scripts ..."
scp $SSH -P "$PORT" distill/train-distill-kitten-v7.jsonl "root@$IP:/workspace/train-distill-kitten-v7.jsonl"
scp $SSH -P "$PORT" distill/train-dpo-kitten-safety.jsonl "root@$IP:/workspace/train-dpo-kitten.jsonl"
scp $SSH -P "$PORT" train-distill-kitten-v7.py "root@$IP:/workspace/train-distill-kitten-v7.py"
scp $SSH -P "$PORT" train-dpo-kitten.py "root@$IP:/workspace/train-dpo-kitten.py"

# SFT (re-create v7) ONCE, then TWO DPO variants (dpoA lr1.5e-5, dpoB lr2e-5, both 2ep) to
# bracket the safety-vs-myth balance. Sequential + detached. Each DPO only runs if SFT succeeded.
echo ">> launching SFT(v7) → DPO-A + DPO-B detached ..."
ssh $SSH -p "$PORT" "root@$IP" \
  'cd /workspace && rm -f train-sft.log train-dpoA.log train-dpoB.log && source venv/bin/activate && setsid nohup bash -c "python train-distill-kitten-v7.py > train-sft.log 2>&1 && [ -f output/distill-sailor-1b-kitten-v7/final-adapter/adapter_config.json ] && DPO_LR=2e-5 DPO_EPOCHS=2 DPO_OUT=/workspace/output/dpoA python train-dpo-kitten.py > train-dpoA.log 2>&1 ; DPO_LR=3e-5 DPO_EPOCHS=2 DPO_OUT=/workspace/output/dpoB python train-dpo-kitten.py > train-dpoB.log 2>&1" < /dev/null & echo "launched pid $!"'

cat <<EOF

================= KITTEN SFT→DPO LIVE: $POD_ID  (root@$IP:$PORT) =================
Watch:  ssh $SSH -p $PORT root@$IP 'tail -f /workspace/train-dpoB.log'  (dpoA done first, then dpoB)
Adapters: /workspace/output/dpoA/final-adapter and /workspace/output/dpoB/final-adapter
