#!/usr/bin/env bash
# ============================================================================
# session_runner.sh — Hiraia ONE-SESSION finetune pipeline (runs ON the pod).
#
...
#
# Usage (on pod):
#   bash /workspace/session_runner.sh
#   LANGS="tagalog" SKIP_TRAIN=1 bash /workspace/session_runner.sh   # eval only
#
# Env knobs:
...
BASE_ID="${BASE_ID:-sail/Sailor2-1B-Chat}"

log(){ echo "[$(date -u +%H:%M:%S)] $*"; }
fail(){ echo "SESSION_FAILED: $*"; exit 1; }

# Per-language config: name|train_script|dataset|output_dir|eval_lang
cfg_for(){ case "$1" in
  tagalog) echo "train-tagalog-unsloth.py|/workspace/output/tagalog-sailor-1b-v3|tagalog";;
  bisaya)  echo "train-bisaya-unsloth.py|/workspace/output/bisaya-sailor-1b-v3|cebuano";;
  *) return 1;; esac; }
...
log "LANGS=[$LANGS] SKIP_TRAIN=$SKIP_TRAIN SKIP_GGUF=$SKIP_GGUF"
...
for L in $LANGS; do
  ...
  ADAPTER_GGUF="$OUTDIR/adapter-$L-f16.gguf"
  log "========== $L =========="
  ...
  if [ "$SKIP_TRAIN" = "1" ] && [ -d "$ADAPTER_DIR" ]; then
    ...
    python -u "/workspace/$TRAIN_PY" 2>&1 | tee "/workspace/train-$L.log"
  ...
  if [ "$SKIP_GGUF" != "1" ]; then
    if [ -f "$QVAC_SRC/convert_lora_to_gguf.py" ]; then
      ...
      ( cd "$QVAC_SRC" && python convert_lora_to_gguf.py "$ADAPTER_DIR" \
          ...
          2>&1 | tee "/workspace/convert-$L.log" \
        && log "[$L] GGUF adapter ready" \
        || log "[$L] WARN: GGUF convert failed — see convert-$L.log"
    else
  ...
  log "[$L] eval (transformers/merged) -> eval-v3-$L.json"
  python /workspace/run_eval_pod_v3.py --model "$MERGED_DIR" --lang "$EVAL_LANG" \
      --out "/workspace/eval-v3-$L.json" 2>&1 | tee "/workspace/eval-$L.log" \
  ...
  if [ "$SKIP_GGUF" != "1" ] && [ -f "$ADAPTER_GGUF" ] && [ -x "$QVAC_BIN/llama-cli" ]; then
    if ensure_base_gguf; then
      log "[$L] eval (GGUF/llama-cli) -> eval-v3-gguf-$L.json"
      LD_LIBRARY_PATH="$QVAC_BIN" python /workspace/run_eval_qvac2_v3.py \
          --cli "$QVAC_BIN/llama-cli" --base "$BASE_GGUF" --adapter "$ADAPTER_GGUF" \
          --lang "$EVAL_LANG" --out "/workspace/eval-v3-gguf-$L.json" \
          2>&1 | tee "/workspace/eval-gguf-$L.log" \
...
for f in /workspace/eval-v3-*.json; do
  [ -f "$f" ] || continue
  echo "--- $f ---"
