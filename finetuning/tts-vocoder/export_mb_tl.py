...
"""
import sys, math
sys.path.insert(0, '/workspace/mbistft')
import numpy as np
...
# ---- model --------------------------------------------------------------------
hps = utils.get_hparams_from_file('/workspace/mbistft/configs/hiraia_tl.json')
net = SynthesizerTrn(len(symbols), hps.data.filter_length//2+1,
                     hps.train.segment_size//hps.data.hop_length, **vars(hps.model))
utils.load_checkpoint('/workspace/mbistft/logs/hiraia_tl2/G_15000.pth', net, None)
net.eval()
...
x1 = ids_for("Ang puso ay nagpapadaloy ng dugo sa buong katawan.")
m1 = torch.ones_like(x1)
x2 = ids_for("Ang buwan ay umiikot sa mundo.")
m2 = torch.ones_like(x2)

import os
os.makedirs('/workspace/onnx-mb-tl', exist_ok=True)

# ---- parity export (noise 0, deterministic) ----------------------------------
wrap0 = Waveform(net, 0.0)
with torch.no_grad():
    ref0 = wrap0(x1, m1)
torch.onnx.export(wrap0, (x1, m1), '/workspace/onnx-mb-tl/model.zero.onnx',
...
s0 = ort.InferenceSession('/workspace/onnx-mb-tl/model.zero.onnx', providers=['CPUExecutionProvider'])
...
torch.onnx.export(wrap, (x1, m1), '/workspace/onnx-mb-tl/model.onnx',
    input_names=['input_ids','attention_mask'], output_names=['waveform'],
    dynamic_axes={'input_ids':{0:'b',1:'t'},'attention_mask':{0:'b',1:'t'},'waveform':{0:'b',1:'s'}},
    opset_version=17, do_constant_folding=True)
s = ort.InferenceSession('/workspace/onnx-mb-tl/model.onnx', providers=['CPUExecutionProvider'])
...
mb = os.path.getsize('/workspace/onnx-mb-tl/model.onnx')/1e6
