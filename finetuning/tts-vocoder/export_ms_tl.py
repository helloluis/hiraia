...
# ---- model --------------------------------------------------------------------
hps = utils.get_hparams_from_file('/workspace/mbistft/configs/hiraia_tl_ms.json')
net = SynthesizerTrn(len(symbols), hps.data.filter_length//2+1,
                     hps.train.segment_size//hps.data.hop_length, **vars(hps.model))
import glob as _g; _ck = sorted(_g.glob('/workspace/mbistft/logs/hiraia_tl_ms/G_*.pth'), key=lambda p:int(p.split('_')[-1].split('.')[0]))[-1]
...
OUT = os.environ.get('OUT_DIR', '/workspace/onnx-ms-tl')
