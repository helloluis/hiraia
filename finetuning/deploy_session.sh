#!/usr/bin/env bash
# ============================================================================
# deploy_session.sh — LOCAL launcher for the one-session finetune+eval pipeline.
#
...
# datasets/scripts/eval, ensures the venv, then kicks off session_runner.sh
...
# registered with the RunPod account, the v3 datasets built under datasets/.
#
# Usage:
#   cd finetuning && ./deploy_session.sh
#   GPU_TYPE="NVIDIA A100 80GB PCIe" ./deploy_session.sh     # pick a different GPU
# ============================================================================
set -euo pipefail
...
DISK_GB="${DISK_GB:-50}"
POD_NAME="${POD_NAME:-hiraia-sailor-v3}"
...
scp $SSH_OPTS -P "$PORT" \
  train-tagalog-unsloth.py train-bisaya-unsloth.py requirements-unsloth.txt session_runner.sh \
  "root@$IP:/workspace/"
scp $SSH_OPTS -P "$PORT" \
  eval/run_eval_pod_v3.py eval/run_eval_qvac2_v3.py eval/student-prompts.json \
  "root@$IP:/workspace/"
scp $SSH_OPTS -P "$PORT" datasets/tagalog/train-v3.jsonl "root@$IP:/workspace/train-tagalog-v3.jsonl"
scp $SSH_OPTS -P "$PORT" datasets/bisaya/train-v3.jsonl  "root@$IP:/workspace/train-bisaya-v3.jsonl"
...
echo ">> launching session_runner.sh detached ..."
ssh $SSH_OPTS -p "$PORT" "root@$IP" \
  'rm -f /workspace/session.log; setsid nohup bash /workspace/session_runner.sh > /workspace/session.log 2>&1 < /dev/null & echo "launched pid $!"'

cat <<EOF

================= POD LIVE: $POD_ID  (root@$IP:$PORT) =================
Watch progress:
  ssh $SSH_OPTS -p $PORT root@$IP 'tail -f /workspace/session.log'

When you see SESSION_DONE, download results + adapters:
  scp $SSH_OPTS -P $PORT root@$IP:'/workspace/eval-v3-*.json' eval/
  scp $SSH_OPTS -P $PORT -r root@$IP:/workspace/output/tagalog-sailor-v3/final-adapter adapters/tagalog-sailor-v3/
  scp $SSH_OPTS -P $PORT -r root@$IP:/workspace/output/bisaya-sailor-v3/final-adapter  adapters/bisaya-sailor-v3/
