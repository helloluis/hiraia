#!/usr/bin/env bash
# ============================================================================
# session_runner_v5.sh — Hiraia ONE-SESSION v5 finetune pipeline (runs ON the pod).
#
# v5 = counterweight SFT rebalance (~146 new rows per language: honest
# uncertainty, anti-confab, brevity, gibberish-clarify, myth verdicts, fast
# [image:] tags, compaction; train-v4 + science-dialogue-v5). Base: sail/Sailor2-3B-Chat.
#
...
#
# Usage (on pod):
#   bash /workspace/session_runner_v5.sh
#   LANGS="tagalog" SKIP_TRAIN=1 bash /workspace/session_runner_v5.sh   # eval only
#
# Env knobs:
...
cfg_for(){ case "$1" in
  tagalog) echo "train-tagalog-unsloth-v5.py|/workspace/output/tagalog-sailor-v5|tagalog";;
  bisaya)  echo "train-bisaya-unsloth-v5.py|/workspace/output/bisaya-sailor-v5|cebuano";;
  *) return 1;; esac; }
...
# ---- disk-quota guard (v4 lesson: quota exhaustion silently truncates writes) ----
# regenerable space hogs from past runs: merged-model dirs + checkpoints
for d in /workspace/output/*/merged-model /workspace/output/*/checkpoint-*; do
  [ -e "$d" ] && { log "freeing regenerable $d"; rm -rf "$d"; }
done
FREE_GB=$(df -BG /workspace | awk 'NR==2 {gsub("G","",$4); print $4}')
log "free on /workspace: ${FREE_GB}GB"
[ "$FREE_GB" -ge 25 ] || fail "only ${FREE_GB}GB free on /workspace (need >=25GB; quota exhaustion silently truncates writes)"
...
for L in $LANGS; do
  ...
  ADAPTER_GGUF="$OUTDIR/adapter-$L-v5-f16.gguf"
  log "========== $L (v5) =========="
  ...
  if [ "$SKIP_TRAIN" = "1" ] && [ -d "$ADAPTER_DIR" ]; then
    ...
    python -u "/workspace/$TRAIN_PY" 2>&1 | tee "/workspace/train-$L-v5.log"
  ...
  if [ "$SKIP_GGUF" != "1" ]; then
    if [ -f "$QVAC_SRC/convert_lora_to_gguf.py" ]; then
      ...
      ( cd "$QVAC_SRC" && python convert_lora_to_gguf.py "$ADAPTER_DIR" \
          ...
          2>&1 | tee "/workspace/convert-$L-v5.log" \
        && log "[$L] GGUF adapter ready" \
        || log "[$L] WARN: GGUF convert failed — see convert-$L-v5.log"
    else
  ...
  log "[$L] eval (transformers/merged) -> eval-v5-$L.json"
  python /workspace/run_eval_pod_v3.py --model "$MERGED_DIR" --lang "$EVAL_LANG" \
      --out "/workspace/eval-v5-$L.json" 2>&1 | tee "/workspace/eval-$L-v5.log" \
  ...
  if [ "$SKIP_GGUF" != "1" ] && [ -f "$ADAPTER_GGUF" ] && [ -x "$QVAC_BIN/llama-cli" ]; then
    if ensure_base_gguf; then
      log "[$L] eval (GGUF/llama-cli) -> eval-v5-gguf-$L.json"
      LD_LIBRARY_PATH="$QVAC_BIN" python /workspace/run_eval_qvac2_v3.py \
          --cli "$QVAC_BIN/llama-cli" --base "$BASE_GGUF" --adapter "$ADAPTER_GGUF" \
          --lang "$EVAL_LANG" --out "/workspace/eval-v5-gguf-$L.json" \
          2>&1 | tee "/workspace/eval-gguf-$L-v5.log" \
...
for f in /workspace/eval-v5-*.json; do
  [ -f "$f" ] || continue
  echo "--- $f ---"
