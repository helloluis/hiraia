...
DISK_GB=60; POD_NAME="hiraia-distill-cat-v11"
...
[ -f distill/train-distill-cat-v11.jsonl ] || { echo "ERR: distill/train-distill-cat-v11.jsonl missing"; exit 1; }
[ -f train-distill-v11.py ] || { echo "ERR: train-distill-v11.py missing"; exit 1; }
...
scp $SSH -P "$PORT" distill/train-distill-cat-v11.jsonl "root@$IP:/workspace/train-distill-v11.jsonl"
scp $SSH -P "$PORT" train-distill-v11.py "root@$IP:/workspace/train-distill-v11.py"

echo ">> launching v9 train detached ..."
ssh $SSH -p "$PORT" "root@$IP" \
  'cd /workspace && rm -f train-v11.log && source venv/bin/activate && setsid nohup python train-distill-v11.py > train-v11.log 2>&1 < /dev/null & echo "launched pid $!"'

cat <<EOF

================= v9 TRAIN LIVE: $POD_ID  (root@$IP:$PORT) =================
Watch:    ssh $SSH -p $PORT root@$IP 'tail -f /workspace/train-v11.log'
Download when done (look for "Adapter saved"):
  scp $SSH -P $PORT -r root@$IP:/workspace/output/distill-sailor-3b-v11/final-adapter adapters/distill-sailor-3b-v11/
