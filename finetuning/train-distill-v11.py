#!/usr/bin/env python3
"""
Train Hiraia v11 LoRA (cat English-leak fix: v9plus + English bucket) for Sailor2-3B (tl/en adapter slot).

v9 = the v6 trainset + a TARGETED safety/myth patch fixing the temp-0.5 negation bug v6 role-play
...
10%-brain myth affirmed). v9 retrains the opener PATTERN on AUP-safe, NON-body yes/no questions:
...
Dataset: finetuning/distill/train-distill-v9.jsonl (~9.8k rows). Same recipe as v4/v5/v6 (r32/a64/3ep).
"""
...
DATASET_PATH = "/workspace/train-distill-v11.jsonl"
OUTPUT_DIR = "/workspace/output/distill-sailor-3b-v11"
...
def main():
    print("🚀 Hiraia v11 LoRA (cat English-leak fix: v9plus + English bucket) (Sailor2-3B) — chitchat/abstention/refusal under the contracted prompt\n")
