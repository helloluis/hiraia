#!/usr/bin/env bash
# ============================================================================
# deploy_v6_distill.sh — provision a GPU pod, upload the v6 dataset+script, and launch the
# skill-only v6 LoRA train DETACHED. Prints monitor/download/terminate commands (no auto-terminate
# — you control billing). v6 = v5 trainset + a TARGETED patch (adjacent-fact abstain, multi-turn
# refusal, firm offscope-help) for the issues v5 role-play surfaced. All on the CONTRACTED prompt
# (train/serve parity). Same recipe (r32/a64/3ep, max_seq 2048). See hiraia-v5-lora-plan.
#
...
[ -f distill/train-distill-v6.jsonl ] || { echo "ERR: distill/train-distill-v6.jsonl missing"; exit 1; }
[ -f train-distill-v6.py ] || { echo "ERR: train-distill-v6.py missing"; exit 1; }
...
if [ -z "$POD_ID" ]; then
  ...
  for CLOUD in SECURE COMMUNITY; do for G in "${GPUS[@]}"; do
    ...
    # which the SSH wait below can never reach (observed 3x on 2026-06-15 v6 deploys).
...
scp $SSH -P "$PORT" distill/train-distill-v6.jsonl "root@$IP:/workspace/train-distill-v6.jsonl"
scp $SSH -P "$PORT" train-distill-v6.py "root@$IP:/workspace/train-distill-v6.py"

echo ">> launching v6 train detached ..."
ssh $SSH -p "$PORT" "root@$IP" \
  'cd /workspace && rm -f train-v6.log && source venv/bin/activate && setsid nohup python train-distill-v6.py > train-v6.log 2>&1 < /dev/null & echo "launched pid $!"'

cat <<EOF

================= v6 TRAIN LIVE: $POD_ID  (root@$IP:$PORT) =================
Watch:    ssh $SSH -p $PORT root@$IP 'tail -f /workspace/train-v6.log'
Download when done (look for "Adapter saved"):
  scp $SSH -P $PORT -r root@$IP:/workspace/output/distill-sailor-3b-v6/final-adapter adapters/distill-sailor-3b-v6/
