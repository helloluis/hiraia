...
import { newId, track, trackMany, telemetryPersona } from './index';
// One logical quiz attempt per displayed page, shared by visibility and grading hooks.
let quiz: { key: number; id: string; shown: boolean; graded: boolean; props?: Props } | undefined;
...
export function showQuiz(key: number, questionId: string, language: string) {
  ...
  if (q.shown) return;
  q.shown = true;
  q.props = { ...telemetryPersona(), attempt_id: q.id, question_id: questionId, language };
  track('quiz_shown', q.props);
}
