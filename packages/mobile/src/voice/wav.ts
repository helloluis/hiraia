...
export function encodeWav(samples: Float32Array, sampleRate: number, trailingSilenceMs = 0): Uint8Array {
  const silenceSamples = Math.round(sampleRate * Math.max(0, trailingSilenceMs) / 1000);
  const sampleCount = samples.length + silenceSamples;
  // The zero-initialized tail is real PCM silence: no JS timer to drift, and
  // stopping/swiping cancels it through the same audio player as the voice.
