...
import { grade8Lessons, planLesson, lessonFactId, lessonsForGrade } from '../src/data/lessonPlan';
...
test('Grade 8 covers all 44 competencies, all categories and every core quiz slot', () => {
  const guide = read('../../../rag/sources/curriculum-guides/matatag-jhs-competencies.json');
  const expected = guide.quarters
    .filter((q: any) => q.grade === 8)
    .flatMap((q: any) => q.competencies.map((c: any) => c.code));
  assert.equal(expected.length, 44);
  assert.deepEqual(new Set(grade8Lessons.flatMap((l) => l.codes)), new Set(expected));
  assert.deepEqual(
    C.curriculumOutline(8).map((l: any) => l.key),
    grade8Lessons.map((l) => l.key)
  );
  ...
  const cross = read('../../../docs/grade8-subcategory-crosswalk.json').mapping;
  const inventory = read('../src/generated/cardsIndex.generated.json').cards;
  assert.deepEqual(
    new Set(cross.map((r: any) => r.subcategory_id)),
    new Set(
      inventory.flatMap((c: any) => (c.cats ?? []).filter((x: string) => x.startsWith('g8-')))
    )
  );
});
test('Grade 8 real feed walks each planned lesson, restores after every card and wraps', () => {
  let cursor = C.curriculumCursor(8, grade8Lessons[0]!.key);
  let current = null;
  ...
  assert.equal(cursor.key, grade8Lessons[0]!.key);
});
test('Grade 8 reserves are all eventually reachable without dropping coverage or duplicating facts', () => {
  for (const lesson of grade8Lessons) {
    const seen = new Set<string>();
    let previous = -1;
...
test('Grade 8 additions are offline, trilingual, scoped correctly and do not activate unsupported grades', () => {
  const extra = read('../src/data/grade8LessonSupplement.json');
  for (const c of extra.cards) {
    for (const lang of ['english', 'tagalog', 'cebuano'])
      assert.ok(C.cardText(C.getCard(c.id), lang).length > 20);
    assert.ok(C.competencyKeys(c.id).every((code: string) => code.startsWith('G8-')));
  }
  ...
  assert.equal(C.curriculumCursor(8, 'Q1.0').key, 'g8:digestion');
  assert.equal(C.curriculumCursor(8, 'g5:matter'), null);
  for (const correction of read('../../../docs/grade8-tag-corrections.json'))
    for (const code of correction.codes) assert.ok(C.competencyKeys(correction.id).includes(code));
  const author = read('../../../rag/pipeline/grade8-lessons.authoring.json');
  for (const lesson of grade8Lessons)
    for (const id of author.excludedCardIds) assert.ok(!lesson.cardIds.includes(id));
  const unit = (key: string, id: string) =>
    grade8Lessons.find((l) => l.key === key)!.units.find((u) => u.id === id)!;
  ...
  for (const [key, id] of [
    ...
    ['g8:land-sea-storms', 'g8-core-land-storm'],
  ]) {
    assert.ok(
      planLesson(grade8Lessons.find((l) => l.key === key)!, new Set()).cards.includes(id),
      id
    );
  }
});
test('Grade 8 supplemental cards and lessons follow the rotated quarter order in real feed weights', () => {
  const guide = read('../../../rag/sources/curriculum-guides/matatag-jhs-competencies.json');
